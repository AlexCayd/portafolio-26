-- =====================================================================
--  alexanderoliva.com — ESQUEMA + DATOS DEL PORTAFOLIO
--  Sirve para montar la base local y para desplegar en producción.
--  Datos: volcado de producción del 15-09-2026 a las 21:17:52.
--
--  ANTES DE CORRERLO EN PRODUCCIÓN
--   1. Descargar un volcado fresco desde phpMyAdmin (es el respaldo). Si
--      tiene datos posteriores al 15-09-2026 (visitas, libros, películas,
--      gym…), pasarlos antes a este archivo: al correrlo, cada tabla del
--      portafolio se BORRA y se vuelve a crear con lo que hay aquí. La
--      excepción es visitas_pagina_dia, que se conserva si ya existe.
--   2. Respaldar public/uploads (no lo toca este script, pero es la otra
--      mitad del contenido).
--   3. phpMyAdmin → seleccionar la base → Importar este archivo.
--      No lleva CREATE DATABASE ni USE: usa la base seleccionada.
--   4. Solo toca las tablas del portafolio (los DROP de abajo). Cualquier
--      otra tabla de la base del hosting queda intacta.
--  Después: subir el código nuevo junto con public/build.
-- =====================================================================

-- Mismos ajustes de sesión que el volcado de phpMyAdmin: los id explícitos
-- se respetan tal cual y los TIMESTAMP se leen en UTC, así no se desplazan
...
--
--  La página interna se arma con secciones numeradas 01..0N, siempre en el
--  mismo orden:
--    01 Contexto y desafío  (columna `contexto`)
--    02 Galería             (proyecto_imagenes, si hay)
--    03..N Secciones        (proyecto_secciones: título + cuerpo libres)
--  Todo lo que no sea el contexto o la galería es una sección: el rol, el
--  stack, el impacto… los escribe el panel con el título que quiera y en el
--  orden que quiera.
-- ---------------------------------------------------------------------
DROP TABLE IF EXISTS proyecto_secciones;
DROP TABLE IF EXISTS proyecto_stack;   -- tabla retirada: el stack es una sección más
...
CREATE TABLE proyectos (
    ...
    contexto    TEXT         NULL,               -- «Contexto y desafío»: abre la ficha
...
-- Secciones de la ficha («Rol y gestión», «Stack», «Impacto y resultados»…).
-- Van después de la galería, en el orden de `orden`. En `cuerpo`, las líneas
-- con forma «Etiqueta: texto» se pintan como tarjetas y el resto como párrafos.
...
CREATE TABLE pys_categorias (
    id           INT AUTO_INCREMENT PRIMARY KEY,
    nombre       VARCHAR(60) NOT NULL UNIQUE,
    admite_serie TINYINT(1)  NOT NULL DEFAULT 0   -- 1 = un título de esta categoría puede ser serie (docuserie, reality…)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE peliculas_series (
    id          INT AUTO_INCREMENT PRIMARY KEY,
    categoria   VARCHAR(60)  NULL,
    es_serie    TINYINT(1)   NOT NULL DEFAULT 0, -- 1 = formato serie: sin duración y con «Creador». Siempre 1 en la categoría «Serie»
    titulo      VARCHAR(160) NOT NULL,
    anio        INT          NULL,
...
-- Inicio (`/`) trae su histórico diario, copiado de `visitas` (que solo se
-- incrementa en la portada). El resto de rutas empieza el día del despliegue:
-- lo anterior solo existe como acumulado sin fechas en visitas_pagina.
-- ÚNICA tabla que NO se tira: en producción se crea antes del despliegue
-- completo para empezar a registrar fechas, y reimportar este archivo no debe
-- borrar lo que ya juntó. (drops.sql sí la tira.)
CREATE TABLE IF NOT EXISTS visitas_pagina_dia (
    id    INT          AUTO_INCREMENT PRIMARY KEY,
...
INSERT INTO `proyectos` (`id`, `titulo`, `slug`, `anio`, `img`, `resumen`, `contexto`, `enlace`, `orden`, `creado`) VALUES
('1', 'Colegio Bilbao', 'colegio-bilbao', '2026', 'proyecto-1786720571-56741a.jpg', 'El Colegio Bilbao requería fortalecer su presencia digital y optimizar la gestión de sus procesos internos. El sistema original presentaba caídas constantes debido a cuellos de botella en un servidor obsoleto (Plesk - Windows) y recursos multimedia no opt', 'El Colegio Bilbao requería fortalecer su presencia digital y optimizar la gestión de sus procesos internos. El sistema original presentaba caídas constantes debido a cuellos de botella en un servidor obsoleto (Plesk - Windows) y recursos multimedia no optimizado. A nivel operativo, el colegio enfrentaba alta fricción en procesos internos, como la gestión y asignación de suplencias docentes', 'https://bilbao.edu.mx', '0', '2026-08-14 15:16:11'),
('2', 'Casa Pestalozzi', 'casa-pestalozzi', '2026', 'proyecto-1786721296-94b3cb.jpg', 'Casa Pestalozzi, un restaurante ubicado en la colonia Del Valle (CDMX), requería una digitalización integral de su flujo de servicio. El reto consistía en conectar la experiencia pública del cliente con la operación interna del negocio, eliinando los cuel', 'Casa Pestalozzi, un restaurante ubicado en la colonia Del Valle (CDMX), requería una digitalización integral de su flujo de servicio. El reto consistía en conectar la experiencia pública del cliente con la operación interna del negocio, eliinando los cuellos de botella entre toma de órdenes, preparación de alimentos, gestión de reservaciones y control administrativo', NULL, '2', '2026-08-14 15:28:16'),
('3', 'Eptos Uno', 'eptos-uno', '2026', 'proyecto-1786722007-d33ae1.jpg', 'El artista Eptos Uno requería un hub digital para consolidar su presencia en línea y conectar directamente con su audiencia. El reto principal era unificar toda su comunicación (fechas de presentaciones, trayectoria en batallas, lanzamientos) en un espaci', 'El artista Eptos Uno requería un hub digital para consolidar su presencia en línea y conectar directamente con su audiencia. El reto principal era unificar toda su comunicación (fechas de presentaciones, trayectoria en batallas, lanzamientos) en un espacio propio, sin depender necesariamente de las redes sociales ni de plataformas de terceros', 'https://eptosuno.com', '0', '2026-08-14 15:40:07'),
('4', 'Casa Bistró Bosque', 'casa-bistro-bosque', '2025', 'proyecto-1786825996-7c48ba.jpg', 'Un complejo residencial con restaurante interno requería una solución digital ágil para gestionar envíos de comida directamente a los departamentos (room service). El reto principal era crear una experiencia de usuario sencilla para los residentes al real', 'Un complejo residencial con restaurante interno requería una solución digital ágil para gestionar envíos de comida directamente a los departamentos (room service). El reto principal era crear una experiencia de usuario sencilla para los residentes al realizar pedidos desde sus dispositivos móviles, y al mismo tiempo, proveer al personal de cocina un sistema de recepción de comandas inmediato, de bajo costo y sin curva de aprendizaje', 'https://casabistrobosque.com', '4', '2026-08-15 20:33:16'),
('5', 'Tonico Vittale', 'tonico-vittale', '2024', 'proyecto-1786826656-4b0b8f.jpg', 'Tonico Vitale, una marca enfocada en el cuidado personal natural, requería consolidar su canal de ventas digital. El reto principal además de darle seguimiento a la tienda en línea transaccional, fue diseñar e implementar estrategias capaces de atraer trá', 'Tonico Vitale, una marca enfocada en el cuidado personal natural, requería consolidar su canal de ventas digital. El reto principal además de darle seguimiento a la tienda en línea transaccional, fue diseñar e implementar estrategias capaces de atraer tráfico cualificado, retener la atención del usuario y maximizar la tasa de conversión y recompra', NULL, '5', '2026-08-15 20:44:16');
...
INSERT INTO `proyecto_secciones` (`id`, `proyecto_id`, `titulo`, `cuerpo`, `orden`) VALUES
('1', '1', 'Rol y Arquitectura', 'Como desarrollador principal, lideré la migración completa del sitio hacia una arquitectura MVC, apoyada en un patrón Active Record para la interacción con bases de datos relacionales. Diseñé y desarrollé desde cero un panel de administración escalable con control de acceso basado en roles con flujos de aprobación de contenidos. Posteriormente, escalé este panel hasta convertirlo en una Intranet a la medida, integrando módulos de operación académica (aulas, profesores, prefectura, horarios, suplencias, entre otros)', '1'),
('2', '1', 'Stack Tecnológico y Soluciones', 'Backend: Arquitectura MVC, PHP, SQL, patrón Active Record\n\nFrontend y UX/UI: Rediseño completo de la landing page y portales internos integrando bibliotecas de animación como GSAP para una experiencia de usuario interactiva y moderna\n\nOptimización y SEO: Refactorización de la estructura de metadatos, encabezados y recursos de carga para optimizar el posicionamiento orgánico y el rendimiento', '2'),
('3', '1', 'Impacto y Resultados', 'Crecimiento exponencial: La optimización técnica y de infraestructura permitió soportar un aumento de tráfico del 320% en un solo mes, procesando más de 356,000 peticiones y cerrando mayo de 2026 con un hito de más de 111,000 visitas acumuladas\n\nEstabilidad: Se eliminaron las caídas del sitio web mediante el cambio de infraestructura y la optimización del peso de los recursos visuales\n\nTransformación digital interna: Se entregó un ecosistema digital completo que además de atender a los visitantes públicos, centraliza la comunicación de estudiantes, familias y colaboradores, resolviendo puntos críticos de la administración escolar', '3'),
('4', '2', 'Rol y Metodología', 'Actué con un rol híbrido como Desarrollador Full-Stack, Project Manager y Scrum Master. Lideré, junto a mi equipo, la concepción del producto, la gestión de los sprints y los recursos del equipo, y diseñé fundamentalmente la experiencia de usuario del sistema interconectado', '1'),
('5', '2', 'Stack Tecnológico y Arquitectura', 'Desarrollo bajo una arquitectura MVC utilizando PHP, bases de datos relacionales SQL implemenando el patrón Active Record, SASS como tecnología de estilos y JavaScript para la reactividad de las interfaces operativas', '2'),
('6', '2', 'Soluciones Implementadas', 'Operación en Tiempo Real: Desarrollo de un Punto de Venta (POS) a la medida sincronizado con cuatro Kitchen Display Systems (KDS) segmentado, lo que permitió orquestar las comandas para las diferentes áreas de la cocina de forma simultánea\n\nGestión y Adquisición: Creación de una landing page comercial conectada a un módulo de feedback para evaluar la satisfacción de los comensales, todo centralizado en un panel de administración para la toma de decisiones gerenciales\n\nInteligencia Artificial: Sistema de recomendaciones en el punto de venta para que los meseros puedan elevar el ticket promedio basado en las preferencias de los comensales. Construcción de insights accionables para el negocio en función de las retroalimentaciones dadas en los feedbacks', '3'),
('7', '3', 'Rol y gestión', 'Como Diseñador UX/UI y Desarrollador Web, dirigí el ciclo completo del proyecto. Traduje la visión del artista en el código y ejecuté la programación de la interfaz garantizando que la estética visual y la identidad de la marca se mantuvieran fieles', '1'),
('8', '3', 'Stack Tecnológico y Soluciones', 'Desarrollo Nativo: Programación ágil utilizando exclusivamente HTML5, CC3 y JavaScript puro para asegurar el máximo rendimiento, logrando un diseño responsivo, fluido y de carga ultrarrápida sin recurrir a librerías externas que añadieran peso innecesario\n\nProcesamiento de Datos en el Cliente: Implementación de lógica a la medida en JavaScript para iterar, ordenar cronológicamente y renderizar de forma dinámica el extenso catálogo de álbumes, detalles de las pistas y lanzamientos históricos del artista, permitiendo una navegación rápida y estructurada por la discografía', '2'),
('9', '3', 'Impacto y Resultados', 'Se entregó un sitio de alto impacto visual que centraliza la carrera del artista. La arquitectura liviana ofrece a los seguidores un punto de acceso directo e ininterrumpido a su música, mercancía y eventos de manera veloz, mejorando significativamente la experiencia del usuario', '3'),
('10', '4', 'Rol y Arquitectura', 'Como Desarrollador, diseñé e implementé una solución basada en microservicios e integraciones en la nube. La arquitectura prescinde de un backend tradicional, comunicando el frontend directamente con herramientas de ofimática (Google Sheets) para automatizar el flujo operativo del restaurante', '1'),
('11', '4', 'Stack Tecnológico y Soluciones', 'Interfaz de Usuario (Frontend): Desarrollo del menú interactivo y carrito de compras utilizando HTML, CSS y JavaScript nativo. Se priorizó un diseño mobile-first de carga rápida para garantizar una excelente experiencia desde los teléfonos de los residentes\n\nAutomatización e Integración: Conexión asíncrona de los pedidos desde la interfaz web hacia Google Sheets vía Webhook. La hoja de cálculo actúa como una base de datos transaccional en la nube y un Kitchen Display System (KDS) en tiempo real, permitiendo a los cocineros visualizar, organizar y despachar las órdenes al instante', '2'),
('12', '4', 'Impacto y Restulados', 'Se logró digitalizar la captación de pedidos departamentales mediante una infraestructura mínima y altamente rentable. La integración eliminó los errores derivados de la toma de órdenes manuales y por teléfono, optimizando los tiempos de preparación y entrega, facilitando la gestión operativa del personal de cocina mediante una herramienta que ya dominaban (hojas de cálculo)', '3'),
('13', '5', 'Rol y Enfoque', 'Como Diseñador UX/UI y Estratega Digital, configuré, desarrollé y personalicé componentes utilizando Shopify. Se diseñó una interfaz intuitiva, responsiva y visualmente atractiva que resalta la identidad de los productos\n\nGrowth y Adquisición: Implementación de estrategias técnicas orientadas a la atracción de clientes y optimización del embudo de ventas, facilitando el recurrido del usuario desde el descubrimiento del producto hasta el checkout', '1'),
('14', '5', 'Impacto y Resultados', 'Se entregó un canal de ventas automatizado y escalable. La plataforma además de exponer el catálogo de productos de manera atractiva, opera activamente como herramienta de retención y conversión, mejorando la visibilidad orgánica de la marca y dinamizando los flujos de ingresos', '2');
...
-- blog_recursos: sin filas en el volcado
...
INSERT INTO `pys_categorias` (`id`, `nombre`, `admite_serie`) VALUES
('3', 'Cortometraje', '0'),
('4', 'Documental', '1'),
('1', 'Película', '0'),
('5', 'Reality', '1'),
...
INSERT INTO `peliculas_series` (`id`, `categoria`, `es_serie`, `titulo`, `anio`, `duracion`, `nota`, `fecha_vista`, `poster`, `comentario`, `seleccion`, `creado`) VALUES
('1', 'Serie', '1', 'El inocente', '2021', NULL, '7.0', '2021-08-14', 'poster-1785528540-f5a454.png', '', '0', '2026-07-24 00:10:50'),
('2', 'Película', '0', 'Ready Player One', '2018', '140', '7.0', '2021-08-18', 'poster-1785528552-571b59.png', '', '0', '2026-07-24 00:12:10'),
('3', 'Película', '0', 'Nace una Estrella', '2018', '136', '9.0', '2021-08-27', 'poster-1785528563-9ccb9e.png', '', '1', '2026-07-28 21:30:23'),
('4', 'Serie', '1', 'The I-Land', '2019', NULL, '2.0', '2021-08-27', 'poster-1785528577-decc5d.png', '', '0', '2026-07-28 21:31:14'),
('5', 'Serie', '1', 'Game of Thrones', '2011', NULL, '9.0', '2021-08-30', 'poster-1785528588-633586.png', '', '0', '2026-07-28 21:32:43'),
('6', 'Película', '0', 'Wonder Woman 1984', '2020', '151', '4.0', '2021-09-02', 'poster-1785528612-3e38d0.png', '', '0', '2026-07-28 21:39:20'),
('7', 'Película', '0', 'Space Jam: A New Legacy', '2021', '105', '4.0', '2021-09-02', 'poster-1785528644-3fee02.png', '', '0', '2026-07-28 21:40:02'),
('8', 'Película', '0', 'Spider-Man: Homecoming', '2017', '133', '8.0', '2021-09-04', 'poster-1785528659-ea5dfe.png', '', '0', '2026-07-28 21:40:57'),
('9', 'Película', '0', 'Los Juegos del Hambre: Sinsajo - Parte I', '2014', '123', '6.0', '2021-09-07', 'poster-1785528682-6884a0.png', '', '0', '2026-07-28 21:42:08'),
('10', 'Película', '0', 'Shang-Chi y la Leyenda de los Diez Anillos', '2021', '132', '7.0', '2021-09-11', 'poster-1785528701-1f7ce5.png', '', '0', '2026-07-28 21:43:42'),
('11', 'Película', '0', 'Megalodón', '2018', '113', '3.0', '2021-09-12', 'poster-1785528752-84519e.png', '', '0', '2026-07-28 21:44:32'),
('12', 'Película', '0', 'Guten Tag, Ramón', '2013', '120', '9.0', '2021-09-12', 'poster-1785528763-59c797.png', '', '1', '2026-07-28 21:45:14'),
('13', 'Película', '0', 'Godzilla vs. Kong', '2021', '113', '6.0', '2021-09-14', 'poster-1785528777-d647b4.png', '', '0', '2026-07-28 21:46:05'),
('14', 'Película', '0', 'Black Widow', '2021', '134', '6.0', '2021-09-16', 'poster-1785528839-32dde7.png', '', '0', '2026-07-28 21:47:43'),
('15', 'Serie', '1', 'Chernobyl', '2019', NULL, '9.0', '2021-09-18', 'poster-1785528856-b8d5e8.png', '', '0', '2026-07-28 21:48:20'),
('16', 'Serie', '1', 'Sex Education', '2019', NULL, '8.0', '2021-09-21', 'poster-1785528933-42fcca.png', '', '0', '2026-07-28 22:05:58'),
('17', 'Película', '0', 'Historia de un Matrimonio', '2019', '137', '10.0', '2021-09-26', 'poster-1785528956-28cb40.png', '', '1', '2026-07-28 22:06:37'),
('18', 'Película', '0', 'Terremoto: La Falla de San Andrés', '2015', '114', '5.0', '2021-09-29', 'poster-1785528966-587cb1.png', '', '0', '2026-07-28 22:07:13'),
('19', 'Película', '0', 'Soul', '2020', '100', '8.0', '2021-09-30', 'poster-1785528977-1e34b6.png', '', '0', '2026-07-28 22:07:50'),
('20', 'Película', '0', 'Charlie y la Fábrica de Chocolate', '2005', '115', '8.0', '2021-10-01', 'poster-1785528988-27b016.png', '', '0', '2026-07-28 22:09:09'),
('21', 'Película', '0', 'The Matrix', '1999', '136', '9.0', '2021-10-03', 'poster-1785529001-5608f0.png', '', '0', '2026-07-28 22:10:06'),
('22', 'Película', '0', 'It', '2017', '135', '8.0', '2021-10-05', 'poster-1785529011-0ea289.png', '', '0', '2026-07-28 22:10:49'),
('23', 'Película', '0', 'The Incredible Hulk', '2008', '112', '6.0', '2021-10-05', 'poster-1785529037-1f02db.png', '', '0', '2026-07-28 22:11:35'),
('24', 'Serie', '1', 'What If...?', '2021', NULL, '6.0', '2021-10-06', 'poster-1785529046-b67e98.png', '', '0', '2026-07-28 22:12:10'),
('25', 'Película', '0', 'The Matrix Reloaded', '2003', '138', '8.0', '2021-10-07', 'poster-1785529056-ba26b9.png', '', '0', '2026-07-28 22:12:47'),
('26', 'Película', '0', 'El Lobo de Wall Street', '2013', '180', '9.0', '2021-10-09', 'poster-1785529066-9abdec.png', '', '1', '2026-07-28 22:13:35'),
('27', 'Película', '0', 'The Lego Movie', '2014', '100', '7.0', '2021-10-09', 'poster-1785529075-8e8c6c.png', '', '0', '2026-07-28 22:14:15'),
('28', 'Película', '0', 'Godzilla', '2014', '123', '6.0', '2021-10-10', 'poster-1785529083-6146bd.png', '', '0', '2026-07-28 22:14:45'),
('29', 'Película', '0', 'Animales Fantásticos y Dónde Encontrarlos', '2016', '132', '7.0', '2021-10-11', 'poster-1785529093-f9e4db.png', '', '0', '2026-07-28 22:15:33'),
('30', 'Serie', '1', 'Días de gallos', '2021', NULL, '4.0', '2021-10-15', 'poster-1785529102-cb4c81.png', '', '0', '2026-07-28 22:16:11'),
('31', 'Película', '0', 'Los Pitufos', '2011', '103', '4.0', '2021-10-17', 'poster-1786081959-416e6e.png', '', '0', '2026-08-07 05:52:39'),
('32', 'Película', '0', 'Oblivion', '2013', '124', '7.0', '2021-10-17', 'poster-1786082294-f698f0.png', '', '0', '2026-08-07 05:58:14'),
('33', 'Serie', '1', 'You', '2018', NULL, '7.0', '2021-10-23', 'poster-1786082330-084bde.png', '', '0', '2026-08-07 05:58:50'),
('34', 'Película', '0', 'El Mesero', '2021', '87', '1.0', '2021-10-24', 'poster-1786082368-1b0a7b.png', '', '0', '2026-08-07 05:59:28'),
('35', 'Película', '0', 'Seven', '1995', '127', '10.0', '2021-10-25', 'poster-1786082401-69327f.png', '', '1', '2026-08-07 06:00:01'),
('36', 'Película', '0', 'Pulp Fiction', '1994', '154', '9.0', '2021-10-30', 'poster-1786082475-32f75b.png', '', '0', '2026-08-07 06:01:15'),
('37', 'Serie', '1', 'Wandavision', '1958', NULL, '8.0', '2021-10-31', 'poster-1786082514-89dda7.png', '', '0', '2026-08-07 06:01:54'),
('38', 'Película', '0', 'Iron Man', '2008', '126', '8.0', '2021-11-01', 'poster-1786082556-3e2f9f.png', '', '0', '2026-08-07 06:02:36'),
('39', 'Película', '0', 'Dune: Part One', '2021', '155', '8.0', '2021-11-01', 'poster-1786082594-6f3824.png', '', '0', '2026-08-07 06:03:14'),
('40', 'Película', '0', 'Spider-Man: Far from Home', '2019', '129', '7.0', '2021-11-01', 'poster-1786082644-3186ba.png', '', '0', '2026-08-07 06:04:04'),
('41', 'Película', '0', 'No se aceptan devoluciones', '2013', '127', '8.0', '2021-11-03', 'poster-1786083733-9e478c.png', '', '0', '2026-08-07 06:21:06'),
('42', 'Película', '0', 'Son como Niños', '2010', '102', '5.0', '2021-11-04', 'poster-1786083717-6b7a86.png', '', '0', '2026-08-07 06:21:57'),
('43', 'Película', '0', 'Náufrago', '2000', '143', '6.0', '2021-11-06', 'poster-1786083787-a65f0a.png', '', '0', '2026-08-07 06:23:07'),
('44', 'Película', '0', 'Eternals', '2021', '156', '6.0', '2021-11-07', 'poster-1786083886-9ce905.png', '', '0', '2026-08-07 06:24:46'),
('45', 'Película', '0', '¿Qué Pasó Ayer?', '2009', '100', '8.0', '2021-11-07', 'poster-1786083935-874c6b.png', '', '0', '2026-08-07 06:25:35'),
('46', 'Película', '0', 'Tren a Busán', '2016', '118', '9.0', '2021-11-09', 'poster-1786083977-59c099.png', '', '1', '2026-08-07 06:26:17'),
('47', 'Película', '0', 'Un Lugar en Silencio', '2018', '90', '9.0', '2021-11-11', 'poster-1786084020-4ec8b0.png', '', '1', '2026-08-07 06:27:00'),
('48', 'Película', '0', 'El Hoyo', '2019', '94', '8.0', '2021-11-13', 'poster-1786084087-e78f5b.png', '', '0', '2026-08-07 06:28:07'),
('49', 'Serie', '1', 'Regular Show', '2010', NULL, '8.0', '2021-11-13', 'poster-1786084122-01853f.png', '', '0', '2026-08-07 06:28:42'),
('50', 'Película', '0', 'Héroe de Centro Comercial', '2009', '91', '2.0', '2021-11-14', 'poster-1786084153-67d274.png', '', '0', '2026-08-07 06:29:13'),
('51', 'Película', '0', 'Beautiful Boy', '2018', '120', '8.0', '2021-11-14', 'poster-1786085271-4d2c9b.png', '', '0', '2026-08-07 06:47:51'),
('52', 'Película', '0', 'Harry Potter y la Piedra Filosofal', '2001', '152', '8.0', '2021-11-14', 'poster-1786085310-a3ef98.png', '', '0', '2026-08-07 06:48:30'),
('53', 'Película', '0', 'Hachiko', '2009', '93', '9.0', '2021-11-15', 'poster-1786085342-20f911.png', '', '1', '2026-08-07 06:49:02'),
('54', 'Película', '0', 'Spider-Man', '2002', '121', '6.0', '2021-11-15', 'poster-1786085377-f4725c.png', '', '0', '2026-08-07 06:49:37'),
('55', 'Película', '0', 'Mirreyes contra Godínez', '2019', '109', '1.0', '2021-11-16', 'poster-1786085423-ef49cb.png', '', '0', '2026-08-07 06:50:23'),
('56', 'Película', '0', 'Spider-Man 2', '2004', '127', '8.0', '2021-11-17', 'poster-1786085462-956d04.png', '', '0', '2026-08-07 06:51:02'),
('57', 'Película', '0', 'Harry Potter y la Cámara de los Secretos', '2002', '161', '7.0', '2021-11-19', 'poster-1786085502-3ac23f.png', '', '0', '2026-08-07 06:51:42'),
('58', 'Película', '0', 'Room', '2015', '118', '9.0', '2021-11-20', 'poster-1786085541-b5bb2f.png', '', '1', '2026-08-07 06:52:21'),
('59', 'Película', '0', 'Son como Niños 2', '2013', '101', '3.0', '2021-11-20', 'poster-1786085575-5ad137.png', '', '0', '2026-08-07 06:52:55'),
('60', 'Película', '0', 'Captain Phillips', '2013', '134', '9.0', '2021-11-21', 'poster-1786085614-0fa1a9.png', '', '1', '2026-08-07 06:53:34'),
('61', 'Película', '0', 'The Founder', '2016', '115', '8.0', '2021-11-21', 'poster-1786086196-bc242b.png', '', '0', '2026-08-07 07:03:16'),
('62', 'Película', '0', '¿Qué Pasó Ayer? Parte II', '2011', '102', '6.0', '2021-11-22', 'poster-1786128128-ddb342.png', '', '0', '2026-08-07 18:42:08'),
('63', 'Película', '0', 'Harry Potter y el Prisionero de Azkabán', '2004', '142', '8.0', '2021-11-23', 'poster-1786128168-7c4248.png', '', '0', '2026-08-07 18:42:48'),
('64', 'Película', '0', 'Jurassic World', '2013', '91', '7.0', '2021-11-26', 'poster-1786128221-aed07b.png', '', '0', '2026-08-07 18:43:41'),
('65', 'Película', '0', 'Spider-Man 3', '2007', '139', '6.0', '2021-11-28', 'poster-1786128263-838379.png', '', '0', '2026-08-07 18:44:23'),
('66', 'Película', '0', 'Kong: Skull Island', '2017', '118', '7.0', '2021-11-28', 'poster-1786128313-568049.png', '', '0', '2026-08-07 18:45:13'),
('67', 'Película', '0', 'The Social Network', '2010', '120', '9.0', '2021-11-30', 'poster-1786128710-6f8e34.png', '', '1', '2026-08-07 18:51:50'),
('68', 'Película', '0', 'Kung Fu Panda', '2011', '90', '6.0', '2021-12-04', 'poster-1786128756-b2f153.png', '', '0', '2026-08-07 18:52:36'),
('69', 'Película', '0', 'After Earth', '2013', '100', '2.0', '2021-12-04', 'poster-1786128847-217795.png', '', '0', '2026-08-07 18:54:07'),
('70', 'Película', '0', 'Kung Fu Panda 2', '2011', '90', '6.0', '2021-12-04', 'poster-1786128881-0e35b8.png', '', '0', '2026-08-07 18:54:41'),
('71', 'Película', '0', 'Captain America: The First Avenger', '2011', '124', '6.0', '2021-12-04', 'poster-1786128924-1216a9.png', '', '0', '2026-08-07 18:55:24'),
('72', 'Película', '0', 'The Dark Night', '2008', '152', '10.0', '2021-12-05', 'poster-1786129087-b47570.png', '', '1', '2026-08-07 18:58:07'),
('73', 'Película', '0', 'La Leyenda de la Llorona', '2011', '75', '7.0', '2021-12-06', 'poster-1786129600-5ac5d5.png', '', '0', '2026-08-07 19:06:40'),
('74', 'Película', '0', 'Alvin y las Ardillas', '2007', '92', '8.0', '2021-12-06', 'poster-1786129710-30a746.png', '', '0', '2026-08-07 19:08:30'),
('75', 'Película', '0', 'Alvin y las Ardillas 2', '2009', '88', '5.0', '2021-12-07', 'poster-1786129746-b3df31.png', '', '0', '2026-08-07 19:09:06'),
('76', 'Película', '0', 'Alvin y las Ardillas 3', '2011', '87', '4.0', '2021-12-07', 'poster-1786129777-7f1e38.png', '', '0', '2026-08-07 19:09:37'),
('77', 'Película', '0', 'The Game', '1997', '129', '8.0', '2021-12-07', 'poster-1786129817-2657fa.png', '', '0', '2026-08-07 19:10:17'),
('78', 'Película', '0', 'Due Date', '2010', '95', '7.0', '2021-12-09', 'poster-1786129847-9ae4f8.png', '', '0', '2026-08-07 19:10:47'),
('79', 'Película', '0', 'tick, tick... BOOM!', '2021', '115', '8.0', '2021-12-10', 'poster-1786129885-742345.png', '', '0', '2026-08-07 19:11:25'),
('80', 'Película', '0', 'Gravity', '2013', '91', '10.0', '2021-11-26', 'poster-1786130010-580aec.png', '', '1', '2026-08-07 19:13:30'),
('81', 'Película', '0', 'The Matrix Revolutions', '2003', '129', '6.0', '2021-12-11', 'poster-1786130059-a77105.png', '', '0', '2026-08-07 19:14:19'),
('82', 'Película', '0', '¿Qué Pasó Ayer? Parte III', '2013', '100', '5.0', '2021-12-12', 'poster-1786130105-5fdbac.png', '', '0', '2026-08-07 19:15:05'),
('83', 'Película', '0', 'Harry Potter y el Cáliz de Fuego', '2005', '157', '8.0', '2021-12-12', 'poster-1786130160-460a72.png', '', '0', '2026-08-07 19:16:00'),
('84', 'Serie', '1', 'El Increíble Mundo de Gumball', '2011', NULL, '7.0', '2021-12-15', 'poster-1786130202-b9c17b.png', '', '0', '2026-08-07 19:16:42'),
('85', 'Película', '0', 'Kung Fu Panda 3', '2016', '95', '5.0', '2021-12-16', 'poster-1786130391-8ef6e3.png', '', '0', '2026-08-07 19:19:51'),
('86', 'Película', '0', 'El Diario de Greg', '2021', '59', '4.0', '2021-12-16', 'poster-1786130899-a2fa9a.png', '', '0', '2026-08-07 19:28:19'),
('87', 'Película', '0', 'Spider-Man: No Way Home', '2021', '148', '8.0', '2021-12-19', 'poster-1786130961-648d3c.png', '', '0', '2026-08-07 19:29:21'),
('88', 'Película', '0', 'Harry Potter y la Orden del Fénix', '2007', '138', '7.0', '2021-12-21', 'poster-1786131004-8995ba.png', '', '0', '2026-08-07 19:30:04'),
('89', 'Serie', '1', 'Hawkeye', '2021', NULL, '7.0', '2021-12-22', 'poster-1786131072-b21e9e.png', '', '0', '2026-08-07 19:31:12'),
('90', 'Película', '0', 'Artificial Intelligence', '2001', '146', '10.0', '2021-12-22', 'poster-1786131110-1d9089.png', '', '1', '2026-08-07 19:31:50'),
('91', 'Película', '0', 'The Matrix Resurrections', '2021', '148', '3.0', '2021-12-24', 'poster-1786131278-49eb20.png', '', '0', '2026-08-07 19:34:38'),
('92', 'Película', '0', 'Harry Potter y el Misterio del Príncipe', '2009', '153', '7.0', '2021-12-25', 'poster-1786131314-579a44.png', '', '0', '2026-08-07 19:35:14'),
('93', 'Película', '0', 'Circle', '2015', '87', '9.0', '2021-12-26', 'poster-1786131362-c111cd.png', '', '1', '2026-08-07 19:36:02'),
('94', 'Película', '0', 'Tau', '2018', '97', '5.0', '2021-12-27', 'poster-1786131395-ee2dbf.png', '', '0', '2026-08-07 19:36:35'),
('95', 'Película', '0', 'Don\'t Look Up', '2021', '138', '9.0', '2021-12-27', 'poster-1786131434-927aab.png', '', '1', '2026-08-07 19:37:14'),
('96', 'Película', '0', 'Camino Amarte', '2017', '93', '3.0', '2021-12-29', 'poster-1786131496-21ce70.png', '', '0', '2026-08-07 19:38:16'),
('97', 'Película', '0', 'Harry Potter y las Reliquias de la Muerte: Parte 1', '2011', '146', '7.0', '2021-12-31', 'poster-1786131551-d5426e.png', '', '0', '2026-08-07 19:39:11'),
('98', 'Película', '0', 'Harry Potter y las Reliquias de la Muerte: Parte 2', '2011', '130', '8.0', '2021-12-31', 'poster-1786131639-b54b17.png', '', '0', '2026-08-07 19:40:39'),
('99', 'Película', '0', 'Mi Villano Favorito', '2010', '95', '8.0', '2022-01-01', 'poster-1786131751-82ad79.png', '', '0', '2026-08-07 19:42:31'),
('100', 'Película', '0', 'Mi Villano Favorito 2', '2013', '98', '7.0', '2022-01-01', 'poster-1786131790-d88f31.png', '', '0', '2026-08-07 19:43:10');
INSERT INTO `peliculas_series` (`id`, `categoria`, `es_serie`, `titulo`, `anio`, `duracion`, `nota`, `fecha_vista`, `poster`, `comentario`, `seleccion`, `creado`) VALUES
('101', 'Película', '0', 'Lo Imposible', '2012', '114', '8.0', '2022-01-02', 'poster-1787017050-78a88e.png', '', '0', '2026-08-17 23:05:04'),
('102', 'Película', '0', 'At Eternity\'s Gate', '2018', '111', '9.0', '2022-01-03', 'poster-1787017063-d70524.png', '', '0', '2026-08-17 23:06:05'),
('103', 'Película', '0', 'Alvin y las Ardillas: Sobre Ruedas', '2015', '92', '4.0', '2022-01-03', 'poster-1787017172-b07641.png', '', '0', '2026-08-17 23:08:08'),
('104', 'Película', '0', 'Nerve', '2016', '96', '3.0', '2022-01-04', 'poster-1787017072-e34026.png', '', '0', '2026-08-18 01:27:44'),
('105', 'Película', '0', 'No Manches Frida', '2016', '114', '3.0', '2022-01-05', 'poster-1787017083-e43292.png', '', '0', '2026-08-18 01:28:58'),
('106', 'Película', '0', 'La Leyenda de la Nahuala', '2007', '82', '5.0', '2022-01-06', 'poster-1787017093-c1be27.png', '', '0', '2026-08-18 01:29:31'),
('107', 'Película', '0', 'Ted', '2012', '106', '6.0', '2022-01-07', 'poster-1787017105-0ecd38.png', '', '0', '2026-08-18 01:30:08'),
('108', 'Película', '0', 'El Secreto de la Calabaza Mágica', '2007', '84', '6.0', '2022-01-08', 'poster-1787017114-b0c5e4.png', '', '0', '2026-08-18 01:30:39'),
('109', 'Película', '0', 'The Maze Runner', '2014', '113', '8.0', '2022-01-09', 'poster-1787017124-c79e82.png', '', '0', '2026-08-18 01:31:20'),
('110', 'Película', '0', 'Maze Runner: Prueba de Fuego', '2015', '131', '7.0', '2022-01-10', 'poster-1787017146-ede8cc.png', '', '0', '2026-08-18 01:31:59'),
('111', 'Película', '0', 'Ted 2', '2015', '115', '4.0', '2022-01-12', 'poster-1787017527-931b11.png', '', '0', '2026-08-18 01:45:27'),
('112', 'Serie', '1', 'Gravity Falls', '2012', NULL, '10.0', '2022-01-12', 'poster-1787017587-badba4.png', '', '1', '2026-08-18 01:46:27'),
('113', 'Película', '0', 'It: Chapter Two', '2019', '169', '8.0', '2022-01-13', 'poster-1787017632-94a6b8.png', '', '0', '2026-08-18 01:47:12'),
('114', 'Película', '0', 'Jaws', '1975', '124', '7.0', '2022-01-14', 'poster-1787017661-f9a455.png', '', '0', '2026-08-18 01:47:41'),
('115', 'Película', '0', 'Tiempo Compartido', '2018', '96', '7.0', '2022-01-14', 'poster-1787017691-d3cf07.png', '', '0', '2026-08-18 01:48:11'),
('116', 'Película', '0', 'Rio', '2011', '96', '9.0', '2022-01-14', 'poster-1787017727-5f439d.png', '', '1', '2026-08-18 01:48:47'),
('117', 'Cortometraje', '0', 'Host', '2020', '57', '7.0', '2022-01-16', 'poster-1787017785-48592f.png', '', '0', '2026-08-18 01:49:45'),
('118', 'Película', '0', 'Venom', '2018', '112', '7.0', '2022-01-17', 'poster-1787017817-cab4b5.png', '', '0', '2026-08-18 01:50:17'),
('119', 'Película', '0', 'Jack and Jill', '2011', '91', '1.0', '2022-01-17', 'poster-1787017855-425b47.png', '', '0', '2026-08-18 01:50:55'),
('120', 'Película', '0', 'Zodiac', '2007', '157', '7.0', '2022-01-19', 'poster-1787018225-85166b.png', '', '0', '2026-08-18 01:57:05'),
('121', 'Película', '0', 'Nosotros los Nobles', '2013', '108', '8.0', '2022-01-19', 'poster-1787018270-3a2bcc.png', '', '0', '2026-08-18 01:57:50'),
('122', 'Serie', '1', 'Euphoria', '2019', NULL, '8.0', '2022-01-20', 'poster-1787018310-7182a6.png', '', '0', '2026-08-18 01:58:30'),
('123', 'Película', '0', 'The Suicide Squad', '2021', '132', '7.0', '2022-01-20', 'poster-1787018348-049003.png', '', '0', '2026-08-18 01:59:08'),
('124', 'Película', '0', 'The Hateful Eight', '2015', '168', '7.0', '2022-01-23', 'poster-1787018381-c4292c.png', '', '0', '2026-08-18 01:59:41'),
('125', 'Película', '0', 'The Shining', '1980', '146', '8.0', '2022-01-23', 'poster-1787018418-67fbfe.png', '', '0', '2026-08-18 02:00:18'),
('126', 'Película', '0', 'Hop', '2011', '95', '3.0', '2022-01-24', 'poster-1787018446-878598.png', '', '0', '2026-08-18 02:00:46'),
('127', 'Reality', '1', 'The Final Table', '2018', NULL, '7.0', '2022-01-24', 'poster-1787018558-cc8671.png', '', '0', '2026-08-18 02:02:38'),
('128', 'Película', '0', 'Dumbo', '1941', '64', '10.0', '2022-01-25', 'poster-1787018603-e1ba4d.png', '', '1', '2026-08-18 02:03:23'),
('129', 'Película', '0', 'El Crimen del Padre Amaro', '2002', '118', '5.0', '2022-01-25', 'poster-1787018635-a191b2.png', '', '0', '2026-08-18 02:03:55'),
('130', 'Película', '0', 'Five Feet Apart', '2019', '116', '6.0', '2022-01-26', 'poster-1787018666-d55bbb.png', '', '0', '2026-08-18 02:04:26'),
('131', 'Película', '0', 'The Witch', '2015', '92', '9.0', '2022-01-26', 'poster-1787018996-4460f1.png', '', '1', '2026-08-18 02:09:56'),
('132', 'Película', '0', 'Miracles from Heaven', '2016', '109', '7.0', '2022-01-26', 'poster-1787019030-8a3e48.png', '', '0', '2026-08-18 02:10:30'),
('133', 'Película', '0', 'The Girl with the Dragon Tattoo', '2011', '158', '8.0', '2022-01-27', 'poster-1787019072-e3fed5.png', '', '0', '2026-08-18 02:11:12'),
('134', 'Película', '0', 'The Angry Birds Movie', '2016', '97', '4.0', '2022-01-27', 'poster-1787019115-0cee7a.png', '', '0', '2026-08-18 02:11:55'),
('135', 'Película', '0', 'Chicuarotes', '2019', '95', '9.0', '2022-01-27', 'poster-1787019148-f887e4.png', '', '1', '2026-08-18 02:12:28'),
('136', 'Película', '0', 'The Karate Kid', '2010', '140', '6.0', '2022-01-28', 'poster-1787019190-81cfd5.png', '', '0', '2026-08-18 02:13:10'),
('137', 'Película', '0', 'Héroe de Centro Comercial 2', '2015', '94', '1.0', '2022-01-30', 'poster-1787019221-cc2af5.png', '', '0', '2026-08-18 02:13:41'),
('138', 'Película', '0', 'Worth', '2020', '118', '8.0', '2022-01-30', 'poster-1787019255-ba5e95.png', '', '0', '2026-08-18 02:14:15'),
('139', 'Película', '0', 'Los Juegos del Hambre', '2012', '142', '7.0', '2022-02-02', 'poster-1787019305-3a5fa4.png', '', '0', '2026-08-18 02:15:05'),
('140', 'Película', '0', 'Gone Girl', '2014', '149', '8.0', '2022-02-05', 'poster-1787019335-b0c6c7.png', '', '0', '2026-08-18 02:15:35'),
('141', 'Película', '0', 'The Power of the Dog', '2021', '126', '8.0', '2022-02-12', 'poster-1787019633-008344.png', '', '0', '2026-08-18 02:20:33'),
('142', 'Serie', '1', 'Clickbait', '2021', NULL, '3.0', '2022-02-12', 'poster-1787019694-bbcb35.png', '', '0', '2026-08-18 02:21:34'),
('143', 'Película', '0', 'Little Women', '2019', '135', '9.0', '2022-02-14', 'poster-1787019729-9b8779.png', '', '1', '2026-08-18 02:22:09'),
('144', 'Película', '0', 'After', '2019', '105', '3.0', '2022-02-14', 'poster-1787019761-da9dfe.png', '', '0', '2026-08-18 02:22:41'),
('145', 'Película', '0', 'Call Me by Your Name', '2017', '132', '10.0', '2022-02-16', 'poster-1787019824-41a59c.png', '', '1', '2026-08-18 02:23:44'),
('146', 'Película', '0', 'Pacific Rim', '2013', '131', '6.0', '2022-02-18', 'poster-1787019858-e0ec73.png', '', '0', '2026-08-18 02:24:18'),
('147', 'Serie', '1', 'Escandalosos', '2014', NULL, '5.0', '2022-02-19', 'poster-1787019892-202ce1.png', '', '0', '2026-08-18 02:24:52'),
('148', 'Película', '0', 'No sé si Cortarme las Venas o Dejármelas Largas', '2013', '103', '1.0', '2022-02-23', 'poster-1787019949-5d540f.png', '', '0', '2026-08-18 02:25:49'),
('149', 'Película', '0', 'Teenage Mutant Ninja Turtles', '2014', '101', '4.0', '2022-02-24', 'poster-1787020000-c0c3e6.png', '', '0', '2026-08-18 02:26:40'),
('150', 'Serie', '1', 'The Cuphead Show', '2005', NULL, '6.0', '2022-02-26', 'poster-1787020045-23cda3.png', '', '0', '2026-08-18 02:27:25'),
('151', 'Película', '0', 'Wall E', '2008', '98', '9.0', '2022-02-28', 'poster-1787149221-71d09d.png', '', '1', '2026-08-19 14:20:21'),
('152', 'Película', '0', 'After We Collided', '2020', '105', '2.0', '2022-03-01', 'poster-1787149253-84935f.png', '', '0', '2026-08-19 14:20:53'),
('153', 'Película', '0', 'Jurassic Park', '1993', '127', '8.0', '2022-03-02', 'poster-1787149289-23f3e8.png', '', '0', '2026-08-19 14:21:29'),
('154', 'Película', '0', 'V for Vendetta', '2005', '132', '9.0', '2022-03-04', 'poster-1787149326-796b35.png', '', '0', '2026-08-19 14:22:06'),
('155', 'Película', '0', 'The Batman', '2022', '176', '10.0', '2022-03-06', 'poster-1787149364-538699.png', '', '1', '2026-08-19 14:22:44'),
('156', 'Película', '0', 'Ex Machina', '2014', '108', '8.0', '2022-03-08', 'poster-1787149405-47423a.png', '', '0', '2026-08-19 14:23:25'),
('157', 'Película', '0', 'It\'s a Kind of a Funny Story', '2010', '101', '7.0', '2022-03-10', 'poster-1787149443-1ae4a3.png', '', '0', '2026-08-19 14:24:03'),
('158', 'Película', '0', 'Sueño en Otro Idioma', '2017', '103', '7.0', '2022-03-10', 'poster-1787149478-3afa52.png', '', '0', '2026-08-19 14:24:38'),
('159', 'Película', '0', 'Turning Red', '2022', '100', '7.0', '2022-03-11', 'poster-1787149513-8def7d.png', '', '0', '2026-08-19 14:25:13'),
('160', 'Película', '0', 'Old', '2021', '108', '6.0', '2022-03-14', 'poster-1787149544-d1b411.png', '', '0', '2026-08-19 14:25:44'),
('161', 'Serie', '1', 'Merlí. Sapere Aude', '2019', NULL, '8.0', '2022-03-15', 'poster-1787149839-b07f08.png', '', '0', '2026-08-19 14:30:39'),
('162', 'Película', '0', 'After We Fell', '2021', '98', '3.0', '2022-03-16', 'poster-1787149870-e99019.png', '', '0', '2026-08-19 14:31:10'),
('163', 'Película', '0', 'No Eres Tú, Soy Yo', '2010', '101', '5.0', '2022-03-18', 'poster-1787149906-f346f3.png', '', '0', '2026-08-19 14:31:46'),
('164', 'Película', '0', 'Sully', '2016', '96', '6.0', '2022-03-20', 'poster-1787149938-c81716.png', '', '0', '2026-08-19 14:32:18'),
('165', 'Serie', '1', 'Scenes from a Marriage', '2021', NULL, '10.0', '2022-03-20', 'poster-1787149988-0bd15c.png', '', '1', '2026-08-19 14:33:08'),
('166', 'Película', '0', 'Jujutsu Kaisen 0', '2021', '112', '6.0', '2022-03-25', 'poster-1787150037-37a1d6.png', '', '0', '2026-08-19 14:33:57'),
('167', 'Película', '0', 'The Space Between Us', '2017', '120', '5.0', '2022-03-31', 'poster-1787150080-7c0e71.png', '', '0', '2026-08-19 14:34:40'),
('168', 'Película', '0', 'The Truman Show', '1998', '103', '9.0', '2022-04-04', 'poster-1787150112-27d4a4.png', '', '1', '2026-08-19 14:35:12'),
('169', 'Película', '0', 'Durante la Tormenta', '2018', '128', '8.0', '2022-04-09', 'poster-1787150158-ef183c.png', '', '0', '2026-08-19 14:35:58'),
('170', 'Película', '0', 'Pokémon: Detective Pikachu', '2019', '104', '6.0', '2022-04-12', 'poster-1787150207-7f8165.png', '', '0', '2026-08-19 14:36:47'),
('171', 'Película', '0', 'Midsommar', '2019', '148', '10.0', '2022-04-13', 'poster-1787150509-6872a3.png', '', '1', '2026-08-19 14:41:49'),
('172', 'Película', '0', '2001: A Space Odyssey', '1968', '149', '7.0', '2022-04-13', 'poster-1787150549-252d50.png', '', '0', '2026-08-19 14:42:29'),
('173', 'Película', '0', 'Cosas Imposibles', '2021', '89', '10.0', '2022-04-15', 'poster-1787150592-8e144b.png', '', '1', '2026-08-19 14:43:12'),
('174', 'Película', '0', 'Dunkirk', '2017', '106', '8.0', '2022-04-15', 'poster-1787150633-9a0816.png', '', '0', '2026-08-19 14:43:53'),
('175', 'Película', '0', '¿Qué Culpa Tiene el Niño?', '2016', '105', '6.0', '2022-04-15', 'poster-1787150699-408ba8.png', '', '0', '2026-08-19 14:44:59'),
('176', 'Película', '0', 'Riddick', '2013', '119', '6.0', '2022-04-16', 'poster-1787150736-67401a.png', '', '0', '2026-08-19 14:45:36'),
('177', 'Película', '0', 'Choose or Die', '2022', '84', '4.0', '2022-04-17', 'poster-1787150773-d3e1f3.png', '', '0', '2026-08-19 14:46:13'),
('178', 'Película', '0', 'Madres Paralelas', '2021', '123', '7.0', '2022-04-17', 'poster-1787150812-33adf7.png', '', '0', '2026-08-19 14:46:52'),
('179', 'Película', '0', 'The Green Knight', '2021', '130', '7.0', '2022-04-19', 'poster-1787150862-3195bf.png', '', '0', '2026-08-19 14:47:42'),
('180', 'Película', '0', 'Death Note', '2017', '101', '1.0', '2022-04-21', 'poster-1787150894-f6c12d.png', '', '0', '2026-08-19 14:48:14'),
('181', 'Stand Up', '0', 'Comedy Central Presenta: Al Cabo es Comedia', '2019', '53', '5.0', '2022-04-21', 'poster-1787151274-c3c68e.png', '', '0', '2026-08-19 14:54:34'),
('182', 'Película', '0', 'Chilangolandia', '2021', '93', '3.0', '2022-04-22', 'poster-1787151317-5f7fb1.png', '', '0', '2026-08-19 14:55:17'),
('183', 'Stand Up', '0', 'Comedy Central Presenta: Machis Mis Huevos', '2019', '49', '7.0', '2022-04-22', 'poster-1787151376-a5d58a.png', '', '0', '2026-08-19 14:56:16'),
('184', 'Serie', '1', 'Inside the World\'s Toughest Prisons', '2016', NULL, '7.0', '2022-04-23', 'poster-1787151422-89a41c.png', '', '0', '2026-08-19 14:57:02'),
('185', 'Película', '0', 'The Two Popes', '2019', '125', '8.0', '2022-04-24', 'poster-1787151460-619f7b.png', '', '0', '2026-08-19 14:57:40'),
('186', 'Película', '0', 'Megamente', '2010', '95', '9.0', '2022-04-24', 'poster-1787151512-7c0c28.png', '', '1', '2026-08-19 14:58:32'),
('187', 'Stand Up', '0', 'Comedy Central Presenta: No Somos Princesas', '2019', '55', '6.0', '2022-04-24', 'poster-1787151554-dbf4a7.png', '', '0', '2026-08-19 14:59:14'),
('188', 'Película', '0', 'The Killing of a Sacred Deer', '2017', '121', '10.0', '2022-04-25', 'poster-1787151589-f149dc.png', '', '1', '2026-08-19 14:59:49'),
('189', 'Película', '0', 'Nocturnal Animals', '2016', '116', '9.0', '2022-04-27', 'poster-1787151633-1fe36f.png', '', '0', '2026-08-19 15:00:33'),
('190', 'Película', '0', 'Sonic the Hedgehog', '2020', '99', '7.0', '2022-05-02', 'poster-1787152074-67a52a.png', '', '0', '2026-08-19 15:07:54'),
('191', 'Película', '0', 'Sonic the Hedgehog 2', '2020', '99', '8.0', '2022-05-02', 'poster-1787152428-9dad38.png', '', '0', '2026-08-19 15:13:48'),
('192', 'Película', '0', 'Doctor Strange in the Multiverse of Madness', '2022', '126', '8.0', '2022-05-04', 'poster-1787152479-44c7ca.png', '', '0', '2026-08-19 15:14:39'),
('193', 'Serie', '1', 'Moon Knight', '2022', NULL, '7.0', '2022-05-04', 'poster-1787152512-44585a.png', '', '0', '2026-08-19 15:15:12'),
('194', 'Película', '0', 'The Do-Over', '2016', '108', '5.0', '2022-05-08', 'poster-1787152542-df3e65.png', '', '0', '2026-08-19 15:15:42'),
('195', 'Película', '0', 'No Time to Die', '2021', '163', '7.0', '2022-05-08', 'poster-1787152572-0b53ed.png', '', '0', '2026-08-19 15:16:12'),
('196', 'Película', '0', 'A Clockwork Orange', '1971', '136', '9.0', '2022-05-09', 'poster-1787152604-d47ffc.png', '', '0', '2026-08-19 15:16:44'),
('197', 'Serie', '1', 'Bar Central', '2016', NULL, '8.0', '2022-05-10', 'poster-1787152641-65bec0.png', '', '0', '2026-08-19 15:17:21'),
('198', 'Película', '0', 'Las Ventajas de Ser Invisible', '2012', '103', '7.0', '2022-05-11', 'poster-1787152697-ce9653.png', '', '0', '2026-08-19 15:18:17'),
('199', 'Película', '0', 'Cuarentones', '2022', '81', '3.0', '2022-05-12', 'poster-1787152737-c4731a.png', '', '0', '2026-08-19 15:18:57'),
('200', 'Película', '0', 'Parasite', '2019', '132', '10.0', '2022-05-13', 'poster-1787152770-c68b97.png', '', '1', '2026-08-19 15:19:30');
INSERT INTO `peliculas_series` (`id`, `categoria`, `es_serie`, `titulo`, `anio`, `duracion`, `nota`, `fecha_vista`, `poster`, `comentario`, `seleccion`, `creado`) VALUES
('201', 'Película', '0', 'Godzilla: King of the Monsters', '2019', '132', '5.0', '2022-05-15', 'poster-1787620039-f67ced.png', '', '0', '2026-08-25 01:07:19'),
('202', 'Película', '0', 'Iron Man 2', '2010', '124', '7.0', '2022-05-19', 'poster-1787620067-54265e.png', '', '0', '2026-08-25 01:07:47'),
('203', 'Película', '0', 'Intensamente', '2015', '95', '10.0', '2022-05-19', 'poster-1787620193-9601f7.png', '', '1', '2026-08-25 01:09:53'),
('204', 'Documental', '1', 'Pixar in Real Life', '2019', NULL, '5.0', '2022-05-19', 'poster-1787620235-2748f7.png', '', '0', '2026-08-25 01:10:35'),
('205', 'Película', '0', 'Life of Pi', '2012', '127', '8.0', '2022-04-20', 'poster-1787620268-611ae7.png', '', '0', '2026-08-25 01:11:08'),
('206', 'Stand Up', '0', 'Alan Saldaña: Mi vida de pobre', '2017', '70', '4.0', '2022-05-22', 'poster-1787620320-12b427.png', '', '0', '2026-08-25 01:12:00'),
('207', 'Cortometraje', '0', 'Party Cloudy', '2009', '6', '8.0', '2022-05-23', 'poster-1787620354-4fd67d.png', '', '0', '2026-08-25 01:12:34'),
('208', 'Documental', '1', 'Inside Pixar', '2020', NULL, '7.0', '2022-05-23', 'poster-1787620632-a09457.png', '', '0', '2026-08-25 01:17:12'),
('209', 'Película', '0', 'Franco Escamilla: Por la anécdota', '2018', '66', '5.0', '2022-05-25', 'poster-1787620666-4da7cf.png', '', '0', '2026-08-25 01:17:46'),
('210', 'Película', '0', 'The Franch Dispatch', '2021', '107', '8.0', '2022-05-27', 'poster-1787620705-e231fd.png', '', '0', '2026-08-25 01:18:25'),
('211', 'Stand Up', '0', 'Daniel Sosa: Sosafado', '2017', '77', '3.0', '2022-05-30', 'poster-1787621042-784b82.png', '', '0', '2026-08-25 01:24:02'),
('212', 'Película', '0', 'Ralph El Demoledor', '2012', '101', '8.0', '2022-06-01', 'poster-1787621078-cc564f.png', '', '0', '2026-08-25 01:24:38'),
('213', 'Documental', '0', 'The Social Dilemma', '2020', '94', '8.0', '2022-06-02', 'poster-1787621130-33217d.png', '', '0', '2026-08-25 01:25:30'),
('214', 'Stand Up', '0', 'Franco Escamilla: Bienvenido al Mundo', '2019', '54', '6.0', '2022-06-02', 'poster-1787621173-0302c6.png', '', '0', '2026-08-25 01:26:13'),
('215', 'Película', '0', 'Jurassic World: Dominion', '2022', '147', '6.0', '2022-06-03', 'poster-1787621207-740898.png', '', '0', '2026-08-25 01:26:47'),
('216', 'Stand Up', '0', 'Coco y Raulito: Carrusel de ternura', '2018', '56', '7.0', '2022-06-04', 'poster-1787621264-9b59c2.png', '', '0', '2026-08-25 01:27:44'),
('217', 'Película', '0', 'Lava', '2014', '7', '7.0', '2022-06-05', 'poster-1787621291-3429c2.png', '', '0', '2026-08-25 01:28:11'),
('218', 'Película', '0', 'Río 2', '2014', '101', '6.0', '2022-06-05', 'poster-1787621320-543e6a.png', '', '0', '2026-08-25 01:28:40'),
('219', 'Película', '0', 'Jojo Rabbit', '2019', '108', '10.0', '2022-06-08', 'poster-1787621348-29289a.png', '', '1', '2026-08-25 01:29:08'),
('220', 'Película', '0', 'Sicario', '2015', '121', '8.0', '2022-06-08', 'poster-1787621375-447f0e.png', '', '0', '2026-08-25 01:29:35'),
('221', 'Stand Up', '0', 'Daniel Sosa: Maleducado', '2019', '54', '4.0', '2022-06-10', 'poster-1787621903-37f576.png', '', '0', '2026-08-25 01:38:23'),
('222', 'Serie', '1', 'Peacemaker', '2022', NULL, '8.0', '2022-06-15', 'poster-1787621931-be7c47.png', '', '0', '2026-08-25 01:38:51'),
('223', 'Película', '0', 'The Northman', '2022', '137', '8.0', '2022-06-16', 'poster-1787621976-34fd51.png', '', '0', '2026-08-25 01:39:36'),
('224', 'Película', '0', 'Everything Everywhere All at Once', '2022', '139', '10.0', '2022-06-17', 'poster-1787622037-1f545b.png', '', '1', '2026-08-25 01:40:37'),
('225', 'Stand Up', '0', 'Alexis de Anda: Mea Culpa', '2017', '60', '5.0', '2022-06-18', 'poster-1787622076-b1b256.png', '', '0', '2026-08-25 01:41:16'),
('226', 'Película', '0', 'Pinocchio', '2022', '105', '4.0', '2022-09-16', 'poster-1787626363-756876.png', '', '0', '2026-08-25 01:41:46'),
('227', 'Película', '0', 'The Lighthouse', '2019', '109', '8.0', '2022-06-20', 'poster-1787622123-508817.png', '', '0', '2026-08-25 01:42:03'),
('228', 'Película', '0', 'Lluvia de Hamburguesas', '2009', '90', '7.0', '2022-06-22', 'poster-1787622191-593534.png', '', '0', '2026-08-25 01:43:11'),
('229', 'Stand Up', '0', 'Lokillo: Nada es Igual', '2021', '63', '6.0', '2022-06-22', 'poster-1787622268-b8cbbb.png', '', '0', '2026-08-25 01:44:28'),
('230', 'Stand Up', '0', 'Dave Chappelle: The Closer', '2021', '72', '7.0', '2022-06-23', 'poster-1787622301-222524.png', '', '0', '2026-08-25 01:45:01'),
('231', 'Serie', '1', 'Mr. Iglesias', '2019', NULL, '7.0', '2022-06-23', 'poster-1787622387-d899c3.png', '', '0', '2026-08-25 01:46:27'),
('232', 'Película', '0', 'Spider-Man: Into the Spider-Verse', '2018', '117', '10.0', '2022-06-25', 'poster-1787622435-5c3761.png', '', '1', '2026-08-25 01:47:15'),
('233', 'Película', '0', 'Top Gun: Maverick', '2022', '130', '7.0', '2022-06-27', 'poster-1787622464-89bf2b.png', '', '0', '2026-08-25 01:47:44'),
('234', 'Stand Up', '0', 'Alex Fernández: El Mejor Comediante del Mundo', '2020', '51', '5.0', '2022-06-27', 'poster-1787622505-5fa6ec.png', '', '0', '2026-08-25 01:48:25'),
('235', 'Película', '0', 'Chip \'n Dale: Rescue Rangers', '2022', '97', '8.0', '2022-06-30', 'poster-1787622544-f091c2.png', '', '0', '2026-08-25 01:49:04'),
('236', 'Película', '0', 'Minions', '2015', '91', '7.0', '2022-07-01', 'poster-1787622578-6c76f4.png', '', '0', '2026-08-25 01:49:38'),
('237', 'Película', '0', 'Minions: The Rise of Gru', '2022', '87', '8.0', '2022-07-02', 'poster-1787622620-215b97.png', '', '0', '2026-08-25 01:50:20'),
('238', 'Reality', '1', 'Zumbo\'s Just Desserts', '2016', NULL, '8.0', '2022-07-02', 'poster-1787622669-293ed2.png', '', '0', '2026-08-25 01:51:09'),
('239', 'Stand Up', '0', 'Ricardo O\'Farrill: Abrazo Navideño', '2016', '31', '6.0', '2022-07-04', 'poster-1787622982-13a577.png', '', '0', '2026-08-25 01:56:22'),
('240', 'Serie', '1', 'Daredevil', '2015', NULL, '9.0', '2022-07-06', 'poster-1787623015-ab98b1.png', '', '0', '2026-08-25 01:56:55'),
('241', 'Película', '0', 'Joker', '2019', '122', '10.0', '2022-07-06', 'poster-1787623396-ffd570.png', '', '1', '2026-08-25 02:03:16'),
('242', 'Película', '0', 'Thor: Love and Thunder', '2022', '118', '8.0', '2022-07-07', 'poster-1787623427-3940e4.png', '', '0', '2026-08-25 02:03:47'),
('243', 'Serie', '1', 'Control Z', '2020', NULL, '5.0', '2022-07-11', 'poster-1787623464-092413.png', '', '0', '2026-08-25 02:04:24'),
('244', 'Serie', '1', 'Marchday: Inside FC Barcelona', '2019', NULL, '7.0', '2022-07-11', 'poster-1787623504-ff8212.png', '', '0', '2026-08-25 02:05:04'),
('245', 'Película', '0', 'Lilo & Stitch', '2002', '85', '7.0', '2022-07-14', 'poster-1787623549-893181.png', '', '0', '2026-08-25 02:05:49'),
('246', 'Película', '0', 'Los Increíbles', '2004', '115', '9.0', '2022-07-15', 'poster-1787623585-8167e0.png', '', '1', '2026-08-25 02:06:25'),
('247', 'Película', '0', 'Los Increíbles 2', '2018', '118', '7.0', '2022-07-16', 'poster-1787623631-b6958e.png', '', '0', '2026-08-25 02:07:11'),
('248', 'Película', '0', 'Vivarium', '2019', '97', '7.0', '2022-07-16', 'poster-1787623666-9b0dec.png', '', '0', '2026-08-25 02:07:46'),
('249', 'Película', '0', 'Elvis', '2022', '159', '5.0', '2022-07-17', 'poster-1787623693-f4230d.png', '', '0', '2026-08-25 02:08:13'),
('250', 'Película', '0', 'The Godfather', '1972', '175', '8.0', '2022-07-19', 'poster-1787623723-2df0c6.png', '', '0', '2026-08-25 02:08:43'),
('251', 'Película', '0', 'Blair WItch', '2016', '89', '1.0', '2022-07-20', 'poster-1787626005-07cf62.png', '', '0', '2026-08-25 02:46:45'),
('252', 'Película', '0', 'Moonlight', '2016', '111', '8.0', '2022-08-02', 'poster-1787626034-856735.png', '', '0', '2026-08-25 02:47:14'),
('253', 'Película', '0', 'Nomadland', '2020', '107', '8.0', '2022-08-10', 'poster-1787626063-165785.png', '', '0', '2026-08-25 02:47:43'),
('254', 'Serie', '1', 'Breaking Bad', '2008', NULL, '10.0', '2022-08-14', 'poster-1787626092-7c751c.png', '', '1', '2026-08-25 02:48:12'),
('255', 'Serie', '1', 'Better Call Saul', '2015', NULL, '10.0', '2022-08-16', 'poster-1787626119-e2edd9.png', '', '1', '2026-08-25 02:48:39'),
('256', 'Película', '0', 'Aquaman', '2018', '143', '6.0', '2022-08-21', 'poster-1787626148-20e2ab.png', '', '0', '2026-08-25 02:49:08'),
('257', 'Película', '0', 'Nope', '2022', '130', '8.0', '2022-08-27', 'poster-1787626174-505c23.png', '', '0', '2026-08-25 02:49:34'),
('258', 'Película', '0', 'Venom: Let There Be Carnage', '2021', '97', '4.0', '2022-08-29', 'poster-1787626205-f5d77a.png', '', '0', '2026-08-25 02:50:05'),
('259', 'Película', '0', 'Fragmentado', '2016', '117', '9.0', '2022-08-30', 'poster-1787626235-54dbbb.png', '', '1', '2026-08-25 02:50:35'),
('260', 'Serie', '1', 'Club de Cuervos', '2015', NULL, '9.0', '2022-09-06', 'poster-1787626272-c6edb1.png', '', '0', '2026-08-25 02:51:12'),
('261', 'Película', '0', 'Dragon Ball Super: Super Hero', '2022', '100', '5.0', '2022-09-10', 'poster-1787626329-66edcc.png', '', '0', '2026-08-25 02:52:09'),
('262', 'Película', '0', 'Get Out', '2017', '104', '9.0', '2022-09-16', 'poster-1787626386-5aae53.png', '', '0', '2026-08-25 02:53:06'),
('263', 'Película', '0', 'Les Misérables', '2012', '158', '8.0', '2022-09-18', 'poster-1787626422-9c4539.png', '', '0', '2026-08-25 02:53:42'),
('264', 'Stand Up', '0', 'Fabrizio Copano: Solo pienso en mí', '2017', '60', '8.0', '2022-09-18', 'poster-1787626463-14d0cc.png', '', '0', '2026-08-25 02:54:23'),
('265', 'Película', '0', 'Don\'t Worry Darling', '2022', '123', '7.0', '2022-10-02', 'poster-1787626498-1bf1ba.png', '', '0', '2026-08-25 02:54:58'),
('266', 'Serie', '1', 'House of the Dragon', '2022', NULL, '9.0', '2022-10-24', 'poster-1787626566-f8ae48.png', '', '0', '2026-08-25 02:56:06'),
('267', 'Stand Up', '0', 'Franco Escamilla: Voyerista Auditivo', '2022', '122', '7.0', '2022-10-25', 'poster-1787626622-c6df43.png', '', '0', '2026-08-25 02:57:02'),
('268', 'Serie', '1', 'Belascoarán', '2022', NULL, '7.0', '2022-10-30', 'poster-1787626651-f92b07.png', '', '0', '2026-08-25 02:57:31'),
('269', 'Stand Up', '0', 'Gabriel Iglesias: Stadium Fluffy', '2022', '115', '8.0', '2022-11-02', 'poster-1787626702-1daa63.png', '', '0', '2026-08-25 02:58:22'),
('270', 'Documental', '1', 'Club América vs Club América', '2022', NULL, '7.0', '2022-11-07', 'poster-1787627299-674b2f.png', '', '0', '2026-08-25 03:08:19'),
('271', 'Película', '0', 'Black Panther: Wakanda Forever', '2022', '161', '7.0', '2022-11-12', 'poster-1787627338-22f005.png', '', '0', '2026-08-25 03:08:58'),
('272', 'Serie', '1', 'I Am Groot', '2022', NULL, '5.0', '2022-11-30', 'poster-1787627372-d3693a.png', '', '0', '2026-08-25 03:09:32'),
('273', 'Cortometraje', '0', 'The Guardians of the Galaxy Holiday Special', '2022', '42', '7.0', '2022-11-30', 'poster-1787627445-2f0b93.png', '', '0', '2026-08-25 03:10:45'),
('274', 'Película', '0', 'Avatar', '2009', '162', '7.0', '2022-12-10', 'poster-1787627491-c69f71.png', '', '0', '2026-08-25 03:11:31'),
('275', 'Película', '0', 'Bones and All', '2022', '131', '6.0', '2022-12-11', 'poster-1787627836-d8de35.png', '', '0', '2026-08-25 03:17:16'),
('276', 'Película', '0', 'Pinocho', '1940', '88', '8.0', '2022-06-19', NULL, '', '0', '2026-08-25 03:22:40'),
('277', 'Stand Up', '0', 'Carlos Ballarta: Falso Profeta', '2021', '63', '8.0', '2022-12-13', 'poster-1787628210-b0504b.png', '', '0', '2026-08-25 03:23:30'),
('278', 'Stand Up', '0', 'Ricardo Quevedo: Hay gente así', '2018', '59', '7.0', '2022-12-15', 'poster-1787628250-4911a3.png', '', '0', '2026-08-25 03:24:10'),
('279', 'Película', '0', 'Avatar: The Way of Water', '2022', '192', '8.0', '2022-12-15', 'poster-1787628304-df9350.png', '', '0', '2026-08-25 03:25:04'),
('280', 'Stand Up', '0', 'Carlos Ballarta: El Amor es de Putos', '2016', '67', '8.0', '2022-12-18', 'poster-1787628348-4c1ce9.png', '', '0', '2026-08-25 03:25:48'),
('281', 'Película', '0', 'Guillermo del Toro\'s Pinnochio', '2022', '117', '9.0', '2022-12-20', 'poster-1787628411-391e1b.png', '', '0', '2026-08-25 03:26:51'),
('282', 'Película', '0', 'Yo, Adolescente', '2019', '97', '5.0', '2022-12-21', 'poster-1787628438-def231.png', '', '0', '2026-08-25 03:27:18'),
('283', 'Stand Up', '0', 'Ricardo Quevedo: Los Amargados Somos Más', '2019', '58', '5.0', '2022-12-22', 'poster-1787628475-918c6c.png', '', '0', '2026-08-25 03:27:55'),
('284', 'Cortometraje', '0', 'Myth: A Frozen Tale', '2019', '8', '4.0', '2022-12-24', 'poster-1787628515-73d404.png', '', '0', '2026-08-25 03:28:35'),
('285', 'Cortometraje', '0', 'Bao', '2018', '8', '9.0', '2022-12-24', 'poster-1787628550-99b601.png', '', '1', '2026-08-25 03:29:10'),
('286', 'Cortometraje', '0', 'Piper', '2016', '6', '9.0', '2022-12-24', 'poster-1787628951-92448a.png', '', '1', '2026-08-25 03:35:51'),
('287', 'Cortometraje', '0', 'Purl', '2018', '8', '10.0', '2022-12-24', 'poster-1787628987-fcbde8.png', '', '1', '2026-08-25 03:36:27'),
('288', 'Cortometraje', '0', 'Float', '2018', '7', '7.0', '2022-12-24', 'poster-1787629026-2d6d37.png', '', '0', '2026-08-25 03:37:06'),
('289', 'Cortometraje', '0', 'Presto', '2008', '5', '7.0', '2022-12-24', 'poster-1787629066-7a8b66.png', '', '0', '2026-08-25 03:37:46'),
('290', 'Película', '0', 'Cómo Ser un Latin Lover', '2017', '115', '6.0', '2022-12-24', 'poster-1787629095-813b14.png', '', '0', '2026-08-25 03:38:15'),
('291', 'Cortometraje', '0', 'Headspace: Unwind Your Mind', '2021', '15', '7.0', '2022-12-24', 'poster-1787629130-871b0a.png', '', '0', '2026-08-25 03:38:50'),
('292', 'Stand Up', '1', 'Zona Rosa', '2019', NULL, '6.0', '2022-12-27', 'poster-1787629171-6a6491.png', '', '0', '2026-08-25 03:39:31'),
('293', 'Stand Up', '0', 'El Especial de Alex Fernández', '2017', '52', '7.0', '2022-12-27', 'poster-1787629217-9418cd.png', '', '0', '2026-08-25 03:40:17'),
('294', 'Documental', '1', 'FIFA Uncovered', '2022', NULL, '8.0', '2022-12-30', 'poster-1787629257-6233d2.png', '', '0', '2026-08-25 03:40:57'),
('295', 'Película', '0', 'Triangle of Sadness', '2022', '147', '10.0', '2022-12-31', 'poster-1787629291-247d03.png', '', '1', '2026-08-25 03:41:31'),
('296', 'Película', '0', 'Lightyear', '2022', '106', '2.0', '2023-01-01', 'poster-1787629339-7c77e9.png', '', '0', '2026-08-25 03:42:19'),
('297', 'Stand Up', '0', 'Capi Pérez: Elotes para Todos', '2022', '62', '1.0', '2023-01-01', 'poster-1787629375-ae841c.png', '', '0', '2026-08-25 03:42:55'),
('298', 'Serie', '1', 'Élite', '2018', NULL, '5.0', '2023-01-04', 'poster-1787629402-dc9652.png', '', '0', '2026-08-25 03:43:22'),
('299', 'Película', '0', 'La La Land', '2016', '128', '8.0', '2023-01-07', 'poster-1787629431-08d18c.png', '', '0', '2026-08-25 03:43:51'),
('300', 'Stand Up', '0', 'Sofía Niño de Rivera: Selección Natural', '2018', '62', '6.0', '2023-01-18', 'poster-1787629467-3bb3ef.png', '', '0', '2026-08-25 03:44:27');
INSERT INTO `peliculas_series` (`id`, `categoria`, `es_serie`, `titulo`, `anio`, `duracion`, `nota`, `fecha_vista`, `poster`, `comentario`, `seleccion`, `creado`) VALUES
('301', 'Película', '0', 'Decision to Leave', '2022', '139', '7.0', '2023-01-21', 'poster-1787851167-a65a89.png', '', '0', '2026-08-27 17:19:27'),
('302', 'Película', '0', 'The Fabelmans', '2022', '151', '9.0', '2023-01-29', 'poster-1787851231-0b6bab.png', '', '0', '2026-08-27 17:20:31'),
('303', 'Serie', '1', 'Smiley', '2022', NULL, '6.0', '2023-02-03', 'poster-1787851269-0d5096.png', '', '0', '2026-08-27 17:21:09'),
('304', 'Película', '0', 'Ant-Man and the Wasp: Quantumania', '2023', '124', '7.0', '2023-02-15', 'poster-1787851329-16e034.png', '', '0', '2026-08-27 17:22:09'),
('305', 'Cortometraje', '0', 'Cuerdas', '2014', '10', '7.0', '2023-02-18', 'poster-1787851369-4e1d9f.png', '', '0', '2026-08-27 17:22:49'),
('306', 'Película', '0', 'Tár', '2022', '158', '9.0', '2023-02-28', 'poster-1787851423-51e64c.png', '', '1', '2026-08-27 17:23:43'),
('307', 'Película', '0', 'The Whale', '2022', '117', '8.0', '2023-03-01', 'poster-1787851464-3bd687.png', '', '0', '2026-08-27 17:24:24'),
('308', 'Serie', '1', 'The Last of Us', '2023', NULL, '9.0', '2023-03-15', 'poster-1787851503-a87858.png', '', '0', '2026-08-27 17:25:03'),
('309', 'Película', '0', 'Creed III', '2023', '116', '6.0', '2023-03-20', 'poster-1787851535-673719.png', '', '0', '2026-08-27 17:25:35'),
('310', 'Stand Up', '0', 'Chris Rock: Selective Outrage', '2023', '69', '6.0', '2023-04-03', 'poster-1787851574-d9cb69.png', '', '0', '2026-08-27 17:26:14'),
('311', 'Película', '0', 'Inside', '2023', '105', '7.0', '2023-04-09', 'poster-1787869552-c77ea3.png', '', '0', '2026-08-27 22:25:52'),
('312', 'Película', '0', 'Guardians of the Galaxy Vol. 3', '2023', '150', '8.0', '2023-05-03', 'poster-1787869596-041a68.png', '', '0', '2026-08-27 22:26:36'),
('313', 'Documental', '1', 'MH370: The Plane That Disappeared', '2023', NULL, '6.0', '2023-05-07', 'poster-1787869639-2fa039.png', '', '0', '2026-08-27 22:27:19'),
('314', 'Documental', '0', 'Stutz', '2022', '96', '7.0', '2023-05-07', 'poster-1787869679-4b4adc.png', '', '0', '2026-08-27 22:27:59'),
('315', 'Documental', '0', 'Hecho en México', '2012', '98', '3.0', '2023-05-17', 'poster-1787869725-609fd4.png', '', '0', '2026-08-27 22:28:45'),
('316', 'Documental', '0', 'Bob Ross: Happy Accidents, Betrayal & Greed', '2021', '92', '5.0', '2023-05-19', 'poster-1787869770-96dc43.png', '', '0', '2026-08-27 22:29:30'),
('317', 'Cortometraje', '0', 'A Tale of Two Kitchens', '2019', '29', '5.0', '2023-05-20', 'poster-1787869811-567092.png', '', '0', '2026-08-27 22:30:11'),
('318', 'Película', '0', 'The Super Mario Bros Movie', '2023', '92', '7.0', '2023-05-20', 'poster-1787869849-f9d832.png', '', '0', '2026-08-27 22:30:49'),
('319', 'Documental', '0', 'Ayotzinapa, El paso de la Tortuga', '2018', '80', '6.0', '2023-05-20', 'poster-1787869893-5e8faa.png', '', '0', '2026-08-27 22:31:33'),
('320', 'Documental', '0', 'The Tinder Swindler', '2022', '114', '7.0', '2023-05-22', 'poster-1787869929-229e48.png', '', '0', '2026-08-27 22:32:09'),
('321', 'Película', '0', 'White Noise', '2022', '136', '4.0', '2023-05-23', 'poster-1787870406-6219be.png', '', '0', '2026-08-27 22:40:06'),
('322', 'Documental', '0', 'Ghislaine Maxwell', '2022', '101', '8.0', '2023-05-24', 'poster-1787870498-e0683b.png', '', '0', '2026-08-27 22:41:38'),
('323', 'Serie', '1', 'La Divina Gula', '2022', NULL, '9.0', '2023-05-24', 'poster-1787870539-8218b9.png', '', '0', '2026-08-27 22:42:19'),
('324', 'Serie', '1', 'Sam & Cat', '2013', NULL, '6.0', '2023-05-28', 'poster-1787870568-71f23d.png', '', '0', '2026-08-27 22:42:48'),
('325', 'Documental', '0', 'American Murder: The Family Next Door', '2020', '83', '7.0', '2023-06-04', 'poster-1787870621-0a8b2e.png', '', '0', '2026-08-27 22:43:41'),
('326', 'Película', '0', 'Spider-Man: Across the Spider-Verse', '2023', '140', '9.0', '2023-06-11', 'poster-1787870662-02b66a.png', '', '0', '2026-08-27 22:44:22'),
('327', 'Reality', '1', 'Ultimate Beastmaster', '2017', NULL, '7.0', '2023-06-13', 'poster-1787870700-ae8f09.png', '', '0', '2026-08-27 22:45:00'),
('328', 'Película', '0', 'The Human Centipede 2', '2011', '91', '1.0', '2023-06-13', 'poster-1787870729-12c06c.png', '', '0', '2026-08-27 22:45:29'),
('329', 'Stand Up', '0', 'Bill Burr: Live at Red Rocks', '2022', '82', '4.0', '2023-06-14', 'poster-1787870768-d4224f.png', '', '0', '2026-08-27 22:46:08'),
('330', 'Película', '0', 'Fast X', '2023', '141', '3.0', '2023-06-20', 'poster-1787870796-bc4eb1.png', '', '0', '2026-08-27 22:46:36'),
('331', 'Película', '0', 'The Flash', '2023', '144', '6.0', '2023-06-20', 'poster-1787871302-088fc6.png', '', '0', '2026-08-27 22:55:02'),
('332', 'Stand Up', '0', 'Alan Saldaña: Encarcelado', '2021', '49', '2.0', '2023-06-22', 'poster-1787871334-765382.jpg', '', '0', '2026-08-27 22:55:34'),
('333', 'Stand Up', '0', 'Carlos Ballarta: Furia Ñera', '2018', '62', '7.0', '2023-06-22', 'poster-1787871352-a2fc45.png', '', '0', '2026-08-27 22:55:52'),
('334', 'Cortometraje', '0', 'If Anything Happens I Love You', '2020', '13', '9.0', '2023-06-22', 'poster-1787871431-f38c4d.png', '', '1', '2026-08-27 22:57:11'),
('335', 'Cortometraje', '0', 'Canvas', '2022', '9', '8.0', '2023-06-27', 'poster-1787871463-075ebc.png', '', '0', '2026-08-27 22:57:43'),
('336', 'Documental', '0', 'Rubius X', '2022', '76', '7.0', '2023-06-27', 'poster-1787871559-20acaa.png', '', '0', '2026-08-27 22:59:19'),
('337', 'Película', '0', 'Batman: The Dark Knight Returns, Part 1', '2012', '76', '7.0', '2023-06-28', 'poster-1787871629-130520.png', '', '0', '2026-08-27 23:00:29'),
('338', 'Película', '0', 'Batman: The Dark Knight Returns, Part 2', '2013', '76', '7.0', '2023-06-29', 'poster-1787871722-a6fcd0.png', '', '0', '2026-08-27 23:02:02'),
('339', 'Cortometraje', '0', 'Forgive Us Our Trespasses', '2022', '13', '7.0', '2023-06-29', 'poster-1787871764-be6766.png', '', '0', '2026-08-27 23:02:44'),
('340', 'Película', '0', 'Hereditary', '2018', '127', '8.0', '2023-07-03', 'poster-1787871814-a637f6.png', '', '0', '2026-08-27 23:03:34'),
('341', 'Película', '0', 'Arrival', '2016', '116', '10.0', '2023-07-07', 'poster-1787872251-8075a9.png', '', '1', '2026-08-27 23:10:51'),
('342', 'Stand Up', '0', 'Liss Pereira: Adulto Promedio', '2022', '63', '4.0', '2023-07-08', 'poster-1787872294-979a73.png', '', '0', '2026-08-27 23:11:34'),
('343', 'Película', '0', 'The Revenant', '2015', '156', '7.0', '2023-07-09', 'poster-1787872329-083150.png', '', '0', '2026-08-27 23:12:09'),
('344', 'Película', '0', 'Alex Strangelove', '2018', '99', '6.0', '2023-07-10', 'poster-1787872365-757370.png', '', '0', '2026-08-27 23:12:45'),
('345', 'Película', '0', 'The Lobster', '2015', '119', '7.0', '2023-07-12', 'poster-1787872398-c51cf2.png', '', '0', '2026-08-27 23:13:18'),
('346', 'Película', '0', 'Mission: Impossible - Dead Reckoning Part One', '2023', '163', '8.0', '2023-07-14', 'poster-1787872442-dd961d.png', '', '0', '2026-08-27 23:14:02'),
('347', 'Película', '0', 'Oppenheimer', '2023', '180', '9.0', '2023-07-24', 'poster-1787872636-1c9bad.png', '', '0', '2026-08-27 23:17:16'),
('348', 'Película', '0', 'Un Lugar en Silencio II', '2020', '97', '8.0', '2023-07-25', 'poster-1787872700-68b7b0.png', '', '0', '2026-08-27 23:18:20'),
('349', 'Documental', '0', 'La Dama del Silencio: El Caso Mataviejitas', '2023', '111', '7.0', '2023-07-28', 'poster-1787872751-1fe714.png', '', '0', '2026-08-27 23:19:11'),
('350', 'Película', '0', 'Barbie', '2023', '114', '7.0', '2023-07-29', 'poster-1787872785-dbb8ae.png', '', '0', '2026-08-27 23:19:45'),
('351', 'Película', '0', 'Ya No Estoy Aquí', '2019', '112', '7.0', '2023-07-31', 'poster-1787874030-585b5e.png', '', '0', '2026-08-27 23:40:30'),
('352', 'Película', '0', 'Talk to Me', '2022', '95', '6.0', '2023-08-11', 'poster-1787874081-4e94e8.png', '', '0', '2026-08-27 23:41:21'),
('353', 'Documental', '1', 'Depp V Heard', '2023', NULL, '7.0', '2023-08-18', 'poster-1787874117-ae4c8f.png', '', '0', '2026-08-27 23:41:57'),
('354', 'Película', '0', 'Darkest Hour', '2017', '125', '7.0', '2023-08-26', 'poster-1787874145-07aca4.png', '', '0', '2026-08-27 23:42:25'),
('355', 'Película', '0', 'Eye in the Sky', '2015', '102', '6.0', '2023-08-28', 'poster-1787874175-c1f9c6.png', '', '0', '2026-08-27 23:42:55'),
('356', 'Película', '0', 'Strays', '2023', '93', '5.0', '2023-09-09', 'poster-1787874213-510a45.png', '', '0', '2026-08-27 23:43:33'),
('357', 'Película', '0', 'Saw X', '2023', '118', '6.0', '2023-10-06', 'poster-1787874249-7d92c7.png', '', '0', '2026-08-27 23:44:09'),
('358', 'Película', '0', 'Five Nights at Freddy\'s', '2023', '109', '4.0', '2023-10-25', 'poster-1787874281-07381f.png', '', '0', '2026-08-27 23:44:41'),
('359', 'Película', '0', 'Women Talking', '2022', '104', '5.0', '2023-10-29', 'poster-1787874312-8d41f4.png', '', '0', '2026-08-27 23:45:12'),
('360', 'Película', '0', 'The Last Airbender', '2010', '103', '6.0', '2023-11-19', 'poster-1787874329-8ae44c.png', '', '0', '2026-08-27 23:45:29'),
('361', 'Serie', '1', 'Ojitos de Huevo', '2023', NULL, '6.0', '2023-11-24', 'poster-1787874724-d6d379.png', '', '0', '2026-08-27 23:52:04'),
('362', 'Película', '0', 'The Killer', '2023', '118', '9.0', '2023-11-25', 'poster-1787874773-c24b88.png', '', '1', '2026-08-27 23:52:53'),
('363', 'Película', '0', 'Los Juegos del Hambre: Balada de Pájaros Cantores y Serpientes', '2023', '157', '7.0', '2023-12-11', 'poster-1787874841-399193.png', '', '0', '2026-08-27 23:54:01'),
('364', 'Película', '0', 'Wonka', '2023', '116', '7.0', '2023-12-11', 'poster-1787874969-7a4d30.png', '', '0', '2026-08-27 23:56:09'),
('365', 'Cortometraje', '0', 'The Wonderful Story of Henry Sugar', '2023', '40', '6.0', '2023-12-16', 'poster-1787875012-5bf033.png', '', '0', '2026-08-27 23:56:52'),
('366', 'Película', '0', 'Napoleon', '2023', '158', '7.0', '2023-12-25', 'poster-1787875044-070487.png', '', '0', '2026-08-27 23:57:24'),
('367', 'Película', '0', 'Totem', '2023', '95', '9.0', '2023-12-27', 'poster-1787875098-f97508.png', '', '1', '2026-08-27 23:58:18'),
('368', 'Serie', '1', 'Merlí', '2015', NULL, '9.0', '2024-01-04', 'poster-1787875192-969242.png', '', '1', '2026-08-27 23:59:52'),
('369', 'Película', '0', 'Aquaman and the Lost Kingdom', '2023', '124', '2.0', '2024-01-05', 'poster-1787875227-392a32.png', '', '0', '2026-08-28 00:00:27'),
('370', 'Película', '0', 'La Sociedad de la Nieve', '2023', '144', '8.0', '2024-01-09', 'poster-1787875259-4a9b42.png', '', '0', '2026-08-28 00:00:59'),
('371', 'Stand Up', '0', 'Ricky Gervais: Armageddon', '2023', '63', '7.0', '2024-01-10', 'poster-1787875592-2c2b56.png', '', '0', '2026-08-28 00:06:32'),
('372', 'Cortometraje', '0', 'Un Abrazo de 3 Minutos', '2019', '28', '2.0', '2024-01-11', 'poster-1787875660-670c8b.png', '', '0', '2026-08-28 00:07:40'),
('373', 'Película', '0', 'Saltburn', '2023', '130', '9.0', '2024-01-13', 'poster-1787876684-7bb9f9.png', '', '1', '2026-08-28 00:24:44'),
('374', 'Película', '0', 'Cuando Acecha la Maldad', '2023', '99', '7.0', '2024-01-19', 'poster-1787876741-461c4a.png', '', '0', '2026-08-28 00:25:41'),
('375', 'Cortometraje', '0', 'Sing', '2016', '25', '6.0', '2024-01-24', 'poster-1787876777-602c8c.png', '', '0', '2026-08-28 00:26:17'),
('376', 'Cortometraje', '0', 'Café Para Llevar', '2014', '13', '5.0', '2024-01-24', 'poster-1787876813-edce3e.png', '', '0', '2026-08-28 00:26:53'),
('377', 'Cortometraje', '0', 'Alternative Math', '2017', '9', '8.0', '2024-01-25', 'poster-1787876846-10aced.png', '', '0', '2026-08-28 00:27:26'),
('378', 'Película', '0', 'Radical', '2023', '125', '9.0', '2024-01-27', 'poster-1787876872-ad7649.png', '', '1', '2026-08-28 00:27:52'),
('379', 'Stand Up', '0', 'Sofía Niño de Rivera: Expuesta', '2016', '80', '6.0', '2024-02-09', 'poster-1787876907-fa7512.png', '', '0', '2026-08-28 00:28:27'),
('380', 'Película', '0', 'Dune: Part Two', '2024', '166', '8.0', '2024-03-02', 'poster-1787876943-bf183b.png', '', '0', '2026-08-28 00:29:03'),
('381', 'Serie', '1', 'Young Royals', '2021', NULL, '9.0', '2024-03-18', 'poster-1787877523-794192.png', '', '1', '2026-08-28 00:38:43'),
('382', 'Cortometraje', '0', 'The After', '2023', '19', '7.0', '2024-03-19', 'poster-1787877564-ede23d.png', '', '0', '2026-08-28 00:39:24'),
('383', 'Película', '0', 'Love, Simon', '2018', '110', '7.0', '2024-03-27', 'poster-1787877578-0d8143.png', '', '0', '2026-08-28 00:39:38'),
('384', 'Cortometraje', '0', 'Hair Love', '2019', '7', '7.0', '2024-04-25', 'poster-1787877659-1406d2.png', '', '0', '2026-08-28 00:40:59'),
('385', 'Documental', '1', 'Together: Treble Winners', '2024', NULL, '8.0', '2024-04-26', 'poster-1787877711-c8cca0.png', '', '0', '2026-08-28 00:41:51'),
('386', 'Película', '0', 'Maze Runner: Cura Mortal', '2018', '143', '7.0', '2024-05-03', 'poster-1787879654-258cae.png', '', '0', '2026-08-28 01:14:14'),
('387', 'Película', '0', 'Close', '2022', '104', '9.0', '2024-05-24', 'poster-1787879702-58adee.png', '', '1', '2026-08-28 01:15:02'),
('388', 'Película', '0', 'Monkey Man', '2024', '121', '7.0', '2024-05-26', 'poster-1787879739-8a2a09.png', '', '0', '2026-08-28 01:15:39'),
('389', 'Stand Up', '0', 'Franco Escamilla: Ladies\' Man', '2024', '74', '7.0', '2024-05-27', 'poster-1787879830-e46f45.png', '', '0', '2026-08-28 01:17:10'),
('390', 'Película', '0', 'Perfect Days', '2023', '124', '10.0', '2024-05-28', 'poster-1787879872-76c62b.png', '', '1', '2026-08-28 01:17:52'),
('391', 'Película', '0', 'Now You See Me', '2013', '115', '7.0', '2024-06-02', 'poster-1787879922-6d0855.png', '', '0', '2026-08-28 01:18:42'),
('392', 'Película', '0', 'Now You See Me 2', '2016', '129', '7.0', '2024-06-07', 'poster-1787879953-71820f.png', '', '0', '2026-08-28 01:19:13'),
('393', 'Cortometraje', '0', 'Strange Way of Life', '2023', '31', '6.0', '2024-06-07', 'poster-1787879984-c7f7e6.png', '', '0', '2026-08-28 01:19:44'),
('394', 'Película', '0', 'Intensamente 2', '2024', '96', '9.0', '2024-06-14', 'poster-1787880058-6546fd.png', '', '0', '2026-08-28 01:20:58'),
('395', 'Serie', '1', 'The 8 Show', '2024', NULL, '7.0', '2024-06-15', 'poster-1787880163-4963c1.png', '', '0', '2026-08-28 01:22:43'),
('396', 'Película', '0', 'Monster', '2023', '127', '7.0', '2024-07-03', 'poster-1787880205-8623c7.png', '', '0', '2026-08-28 01:23:25'),
('397', 'Película', '0', 'Mi Villano Favorito 4', '2024', '94', '6.0', '2024-07-08', 'poster-1787880248-8f08e6.png', '', '0', '2026-08-28 01:24:08'),
('398', 'Película', '0', 'Un Lugar en Silencio: Día Uno', '2024', '99', '6.0', '2024-07-09', 'poster-1787880297-d31ea5.png', '', '0', '2026-08-28 01:24:57'),
('399', 'Película', '0', 'Aftersun', '2022', '102', '8.0', '2024-07-23', 'poster-1787880384-0f87a5.png', '', '0', '2026-08-28 01:26:24'),
('400', 'Película', '0', 'Deadpool & Wolverine', '2024', '128', '7.0', '2024-07-24', 'poster-1787880425-9ffaf3.png', '', '0', '2026-08-28 01:27:05');
INSERT INTO `peliculas_series` (`id`, `categoria`, `es_serie`, `titulo`, `anio`, `duracion`, `nota`, `fecha_vista`, `poster`, `comentario`, `seleccion`, `creado`) VALUES
('401', 'Película', '0', 'EO', '2022', '88', '3.0', '2024-07-25', 'poster-1788966355-89fef7.png', '', '0', '2026-09-09 15:05:55'),
('402', 'Cortometraje', '0', 'A Trip to the Moon', '1902', '13', '7.0', '2024-07-25', 'poster-1788966411-1cdcfe.png', '', '0', '2026-09-09 15:06:51'),
('403', 'Película', '0', 'Glass', '2019', '129', '5.0', '2024-08-03', 'poster-1788966446-65d39c.png', '', '0', '2026-09-09 15:07:26'),
('404', 'Película', '0', 'Alien: Romulus', '2024', '119', '8.0', '2024-08-30', 'poster-1788966488-180b20.png', '', '0', '2026-09-09 15:08:08'),
('405', 'Película', '0', 'The Wild Robot', '2024', '102', '9.0', '2024-09-30', 'poster-1788966535-6643d4.png', '', '1', '2026-09-09 15:08:55'),
('406', 'Reality', '1', 'Ramsay\'s Kitchen Nightmares USA', '2007', NULL, '8.0', '2024-10-01', 'poster-1788966591-1ef715.png', '', '0', '2026-09-09 15:09:51'),
('407', 'Película', '0', 'El Hoyo 2', '2024', '99', '7.0', '2024-10-05', 'poster-1788966655-21a54d.png', '', '0', '2026-09-09 15:10:55'),
('408', 'Serie', '1', 'Heartstopper', '2010', NULL, '9.0', '2024-11-02', 'poster-1788966692-3b0f26.png', '', '1', '2026-09-09 15:11:32'),
('409', 'Documental', '0', 'El Secreto del Doctor Grinberg', '2020', '91', '8.0', '2024-11-18', 'poster-1788966725-926686.png', '', '0', '2026-09-09 15:12:05'),
('410', 'Película', '0', 'We Live in Time', '2024', '108', '7.0', '2024-11-23', 'poster-1788966752-6e5c87.png', '', '0', '2026-09-09 15:12:32'),
('411', 'Película', '0', 'Sonic the Hedgehog 3', '2024', '110', '8.0', '2024-12-30', 'poster-1788967017-0c599a.png', '', '0', '2026-09-09 15:16:57'),
('412', 'Película', '0', 'Flow', '2024', '85', '6.0', '2025-01-01', 'poster-1788967045-0b817a.png', '', '0', '2026-09-09 15:17:25'),
('413', 'Película', '0', 'The Grinch', '2018', '86', '6.0', '2025-01-03', 'poster-1788967071-a4606e.png', '', '0', '2026-09-09 15:17:51'),
('414', 'Película', '0', 'Civil War', '2024', '109', '7.0', '2025-01-17', 'poster-1788967130-d2d7fe.png', '', '0', '2026-09-09 15:18:50'),
('415', 'Serie', '1', 'LOL: Last One Laughing', '2018', NULL, '6.0', '2025-01-17', 'poster-1788967177-8ca083.png', '', '0', '2026-09-09 15:19:37'),
('416', 'Película', '0', 'Late Night with the Devil', '2023', '93', '9.0', '2025-01-26', 'poster-1788967218-d581da.png', '', '1', '2026-09-09 15:20:18'),
('417', 'Stand Up', '0', 'Gabriel Iglesias: Legend of Fluffy', '2025', '101', '6.0', '2025-02-01', 'poster-1788967263-4a3ebc.png', '', '0', '2026-09-09 15:21:03'),
('418', 'Película', '0', 'Godzilla X Kong: The New Empire', '2024', '115', '3.0', '2025-02-03', 'poster-1788967303-199832.png', '', '0', '2026-09-09 15:21:43'),
('419', 'Película', '0', 'Los Dos Hemisferios de Lucca', '2025', '96', '5.0', '2025-02-07', 'poster-1788967341-716df3.png', '', '0', '2026-09-09 15:22:21'),
('420', 'Documental', '0', 'La Oscuridad de la Luz del Mundo', '2023', '113', '6.0', '2025-02-08', 'poster-1788967372-0b8ff3.png', '', '0', '2026-09-09 15:22:52'),
('421', 'Serie', '1', 'Culinary Class Wars', '2024', NULL, '8.0', '2025-02-16', 'poster-1788968142-c09e15.png', '', '0', '2026-09-09 15:35:42'),
('422', 'Serie', '1', 'Five Star Chef', '2023', NULL, '7.0', '2025-02-27', 'poster-1788968171-86bc85.png', '', '0', '2026-09-09 15:36:11'),
('423', 'Película', '0', 'Mickey 17', '2025', '137', '8.0', '2025-03-07', 'poster-1788968204-730277.png', '', '0', '2026-09-09 15:36:44'),
('424', 'Serie', '1', 'Cassandra', '2025', NULL, '6.0', '2025-03-08', 'poster-1788968228-b353c9.png', '', '0', '2026-09-09 15:37:08'),
('425', 'Película', '0', 'The Substance', '2024', '141', '6.0', '2025-03-09', 'poster-1788968271-a341e7.png', '', '0', '2026-09-09 15:37:51'),
('426', 'Serie', '1', 'Gordon Ramsay\'s 24 Hours to Hell and Back', '2018', NULL, '8.0', '2025-03-14', 'poster-1788968310-38d10a.png', '', '0', '2026-09-09 15:38:30'),
('427', 'Película', '0', 'Señora Influencer', '2023', '100', '6.0', '2025-03-14', 'poster-1788968347-6f22e6.png', '', '0', '2026-09-09 15:39:07'),
('428', 'Película', '0', 'Cónclave', '2024', '120', '8.0', '2025-03-15', 'poster-1788968377-42ed19.png', '', '0', '2026-09-09 15:39:37'),
('429', 'Película', '0', '500 Días de Escobar', '2023', '98', '5.0', '2025-03-30', 'poster-1788968413-dd5081.png', '', '0', '2026-09-09 15:40:13'),
('430', 'Serie', '1', 'Adolescence', '2025', NULL, '8.0', '2025-04-05', 'poster-1788968483-dc70c6.png', '', '0', '2026-09-09 15:41:23'),
('431', 'Documental', '1', 'Narco Circo', '2023', NULL, '8.0', '2025-04-11', 'poster-1788968519-8f6605.png', '', '0', '2026-09-09 15:41:59'),
('432', 'Documental', '1', 'Quiet on Set: The Dark Side of Kids TV', '2024', NULL, '8.0', '2025-04-24', 'poster-1788968561-5ced62.png', '', '0', '2026-09-09 15:42:41'),
('433', 'Película', '0', 'The Day the Earth Blew Up: A Looney Tunes Movie', '2024', '91', '5.0', '2025-04-25', 'poster-1788968619-deadb3.png', '', '0', '2026-09-09 15:43:39'),
('434', 'Documental', '0', 'El papa Francisco: un hombre de palabra', '2018', '96', '7.0', '2025-04-26', 'poster-1788968664-a34919.png', '', '0', '2026-09-09 15:44:24'),
('435', 'Película', '0', 'Thunderbolts*', '2025', '127', '8.0', '2025-05-02', 'poster-1788968738-7ae8d6.png', '', '0', '2026-09-09 15:45:38'),
('436', 'Documental', '0', 'Tutankamon: El último viaje', '2022', '90', '4.0', '2025-05-04', 'poster-1788968802-191363.png', '', '0', '2026-09-09 15:46:42'),
('437', 'Documental', '0', 'Radio Silence', '2019', '79', '8.0', '2025-05-15', 'poster-1788968835-962a7a.png', '', '0', '2026-09-09 15:47:15'),
('438', 'Serie', '1', 'Black Mirror', '2011', NULL, '7.0', '2025-05-23', 'poster-1788968897-6d62bb.png', '', '1', '2026-09-09 15:48:17'),
('439', 'Película', '0', 'Karate Kid: Legends', '2025', '94', '8.0', '2025-05-25', 'poster-1788968935-479aa5.png', '', '0', '2026-09-09 15:48:55'),
('440', 'Película', '0', 'Mission: Impossible - The Final Reckoning', '2025', '169', '8.0', '2025-05-25', 'poster-1788969004-16157a.png', '', '0', '2026-09-09 15:50:04'),
('441', 'Stand Up', '0', 'Sofía Niño de Rivera: Lo Volvería a Hacer', '2022', '55', '2.0', '2025-05-25', 'poster-1788969324-2f634d.png', '', '0', '2026-09-09 15:55:24'),
('442', 'Serie', '1', 'Libre de Reír', '2023', NULL, '9.0', '2025-05-25', 'poster-1788969354-45c463.png', '', '0', '2026-09-09 15:55:54'),
('443', 'Película', '0', 'Bardo: Falsa Crónica de unas Cuantas Verdades', '2022', '159', '10.0', '2025-05-29', 'poster-1788969394-fc6aac.png', '', '1', '2026-09-09 15:56:34'),
('444', 'Película', '0', 'Avengers: Infinity War', '2018', '149', '8.0', '2025-06-03', 'poster-1788969438-a63f2b.png', '', '0', '2026-09-09 15:57:18'),
('445', 'Película', '0', 'Fantasia', '1940', '124', '5.0', '2025-06-05', 'poster-1788969474-9401ba.png', '', '0', '2026-09-09 15:57:54'),
('446', 'Cortometraje', '0', '22 vs Earth', '2021', '6', '4.0', '2025-06-05', 'poster-1788969505-fe2387.png', '', '0', '2026-09-09 15:58:25'),
('447', 'Película', '0', 'Mission: Impossible', '1996', '110', '7.0', '2025-06-11', 'poster-1788969543-c32209.png', '', '0', '2026-09-09 15:59:03'),
('448', 'Película', '0', 'Mission: Impossible II', '2000', '123', '7.0', '2025-06-12', 'poster-1788969577-b22bac.png', '', '0', '2026-09-09 15:59:37'),
('449', 'Película', '0', 'Cyber Hell: Exposing an Internet Horror', '2022', '105', '6.0', '2025-06-12', 'poster-1788969635-50656a.png', '', '0', '2026-09-09 16:00:35'),
('450', 'Película', '0', 'Cómo Entrenar a tu Dragón', '2025', '125', '7.0', '2025-06-14', 'poster-1788969787-095240.png', '', '0', '2026-09-09 16:03:07'),
('451', 'Película', '0', 'Cómo Entrenar a tu Dragón 2', '2014', '102', '7.0', '2025-06-15', 'poster-1788972243-ba9fb2.png', '', '0', '2026-09-09 16:44:03'),
('452', 'Serie', '1', 'El Juego del Calamar', '2021', NULL, '7.0', '2025-07-08', 'poster-1788972295-00bac1.png', '', '0', '2026-09-09 16:44:55'),
('453', 'Película', '0', 'F1', '2025', '155', '7.0', '2025-07-08', 'poster-1788972323-94a705.png', '', '0', '2026-09-09 16:45:23'),
('454', 'Película', '0', 'Superman', '2025', '129', '8.0', '2025-07-11', 'poster-1788972357-929ae3.png', '', '0', '2026-09-09 16:45:57'),
('455', 'Película', '0', 'Mission: Impossible III', '2006', '126', '7.0', '2025-07-15', 'poster-1788972390-341086.png', '', '0', '2026-09-09 16:46:30'),
('456', 'Película', '0', 'Los Juegos del Hambre: En Llamas', '2013', '146', '7.0', '2025-07-20', 'poster-1788972426-28d7d9.png', '', '0', '2026-09-09 16:47:06'),
('457', 'Película', '0', 'Jurassic World: Rebirth', '2025', '133', '7.0', '2025-07-22', 'poster-1788972468-38237c.png', '', '0', '2026-09-09 16:47:48'),
('458', 'Película', '0', 'Tony Robbins: I Am Not Your Guru', '2016', '115', '7.0', '2025-07-24', 'poster-1788972515-859697.png', '', '0', '2026-09-09 16:48:35'),
('459', 'Película', '0', 'Los Juegos del Hambre: Sinsajo - Parte 2', '2015', '137', '7.0', '2025-07-27', 'poster-1788972569-851685.png', '', '0', '2026-09-09 16:49:29'),
('460', 'Película', '0', 'The Fantastic Four: First Steps', '2025', '115', '8.0', '2025-07-29', 'poster-1788972627-e29b03.png', '', '0', '2026-09-09 16:50:27'),
('461', 'Película', '0', 'Her', '2013', '126', '8.0', '2025-07-31', 'poster-1788972677-f8386f.png', '', '0', '2026-09-09 16:51:17'),
('462', 'Película', '0', 'La Cocina', '2024', '139', '8.0', '2025-08-01', 'poster-1788972707-25f99c.png', '', '0', '2026-09-09 16:51:47'),
('463', 'Película', '0', 'The Bad Guys 2', '2025', '104', '8.0', '2025-08-05', 'poster-1788972748-aab079.png', '', '0', '2026-09-09 16:52:29'),
('464', 'Película', '0', 'Materialists', '2025', '116', '7.0', '2025-08-08', 'poster-1788972774-1223ba.png', '', '0', '2026-09-09 16:52:54'),
('465', 'Película', '0', 'Bring Her Back', '2025', '104', '8.0', '2025-08-31', 'poster-1788972820-e6a583.png', '', '0', '2026-09-09 16:53:40'),
('466', 'Película', '0', 'Doctor Strange', '2016', '115', '7.0', '2025-09-10', 'poster-1788972868-5cfcb6.png', '', '0', '2026-09-09 16:54:28'),
('467', 'Película', '0', 'Ratatouille', '2007', '111', '8.0', '2025-09-13', 'poster-1788972895-939b32.png', '', '0', '2026-09-09 16:54:55'),
('468', 'Película', '0', 'One Battle After Another', '2025', '161', '7.0', '2025-09-28', 'poster-1788972927-285ebf.png', '', '0', '2026-09-09 16:55:27'),
('469', 'Serie', '1', 'La Casa de Papel', '2017', NULL, '9.0', '2023-10-03', 'poster-1788972960-72d2cc.png', '', '1', '2026-09-09 16:56:00'),
('470', 'Película', '0', 'Tron: Ares', '2025', '119', '4.0', '2025-10-24', 'poster-1788972999-4920cd.png', '', '0', '2026-09-09 16:56:39'),
('471', 'Película', '0', 'After the Hunt', '205', '138', '8.0', '2025-10-24', 'poster-1788973058-d39ed0.png', '', '0', '2026-09-09 16:57:38'),
('472', 'Película', '0', 'Now You See Me: Now You Don\'t', '2025', '113', '6.0', '2025-11-15', 'poster-1788973150-37fb71.png', '', '0', '2026-09-09 16:59:10'),
('473', 'Película', '0', 'Avengers: Age of Ultron', '2015', '141', '7.0', '2025-11-28', 'poster-1788973184-723c0b.png', '', '0', '2026-09-09 16:59:44'),
('474', 'Película', '0', 'Frankenstein', '2025', '149', '8.0', '2025-11-28', 'poster-1788973214-b3468c.png', '', '0', '2026-09-09 17:00:14'),
('475', 'Película', '0', 'Weapons', '2025', '128', '9.0', '2025-11-30', 'poster-1788973242-7dc7a5.png', '', '1', '2026-09-09 17:00:42'),
('476', 'Película', '0', 'My Oxford Year', '2025', '112', '4.0', '2025-12-05', 'poster-1788973276-0071cf.png', '', '0', '2026-09-09 17:01:16'),
('477', 'Película', '0', 'It Was Just an Accident', '2025', '103', '7.0', '2025-12-06', 'poster-1788973312-2a03fd.png', '', '0', '2026-09-09 17:01:52'),
('478', 'Película', '0', 'Bugonia', '2025', '118', '9.0', '2025-12-07', 'poster-1788973337-ec7d8f.png', '', '1', '2026-09-09 17:02:17'),
('479', 'Película', '0', 'Five Nights at Freddy\'s 2', '2025', '104', '2.0', '2025-12-09', 'poster-1788973393-64b67f.png', '', '0', '2026-09-09 17:03:13'),
('480', 'Película', '0', 'Zootopia', '2016', '108', '6.0', '2025-12-11', 'poster-1788973421-a754fe.png', '', '0', '2026-09-09 17:03:41'),
('481', 'Película', '0', 'Another End', '2024', '119', '7.0', '2025-12-14', 'poster-1788973461-9baaf1.png', '', '0', '2026-09-09 17:04:21'),
('482', 'Película', '0', 'Avatar: Fire and Ash', '2025', '197', '7.0', '2025-12-19', 'poster-1788973488-5828d4.png', '', '0', '2026-09-09 17:04:48'),
('483', 'Cortometraje', '0', 'Tlalnepantla', '2025', '6', '0.0', '2025-12-24', 'poster-1788973512-d37e15.png', '', '0', '2026-09-09 17:05:12'),
('484', 'Película', '0', 'La Hora de los Valientes', '2025', '107', '7.0', '2025-12-25', 'poster-1788973537-962381.png', '', '0', '2026-09-09 17:05:37'),
('485', 'Película', '0', 'Sentimental Value', '2025', '135', '8.0', '2025-12-27', 'poster-1788973561-70f9c2.png', '', '0', '2026-09-09 17:06:01'),
('486', 'Serie', '1', 'Vis a Vis', '2015', NULL, '8.0', '2025-12-30', 'poster-1788973590-514530.png', '', '0', '2026-09-09 17:06:30'),
('487', 'Serie', '1', 'De Viaje con los Derbez', '2019', NULL, '6.0', '2026-01-01', 'poster-1788973619-e51881.png', '', '0', '2026-09-09 17:06:59'),
('488', 'Película', '0', 'The Menu', '2022', '107', '8.0', '2026-01-02', 'poster-1788973644-686ae1.png', '', '0', '2026-09-09 17:07:24'),
('489', 'Stand Up', '0', 'What\'s in a Name?', '2022', '39', '6.0', '2026-01-02', 'poster-1788973675-d246c4.png', '', '0', '2026-09-09 17:07:55'),
('490', 'Película', '0', 'Rental Family', '2025', '109', '9.0', '2026-01-08', 'poster-1788973706-cbdf28.png', '', '1', '2026-09-09 17:08:26'),
('491', 'Película', '0', 'Anora', '2024', '139', '8.0', '2026-01-11', 'poster-1788973764-e09de3.png', '', '0', '2026-09-09 17:09:24'),
('492', 'Película', '0', 'No Other Choice', '2025', '139', '8.0', '2026-01-16', 'poster-1788973837-64e64e.png', '', '0', '2026-09-09 17:10:37'),
('493', 'Película', '0', 'Marty Supreme', '2025', '149', '9.0', '2026-01-17', 'poster-1788973866-3a64fc.png', '', '0', '2026-09-09 17:11:06'),
('494', 'Película', '0', 'Sobriedad, Me Estás Matando', '2025', '108', '3.0', '2026-01-30', 'poster-1788973900-2654a4.png', '', '0', '2026-09-09 17:11:40'),
('495', 'Serie', '1', 'LOL Buscando Talento: México', '2026', NULL, '2.0', '2026-01-30', 'poster-1788973924-cdcdb7.png', '', '0', '2026-09-09 17:12:04'),
('496', 'Película', '0', 'Hamnet', '2025', '125', '8.0', '2026-02-01', 'poster-1788973951-45c1ba.png', '', '0', '2026-09-09 17:12:31'),
('497', 'Documental', '0', 'Buy Now! The Shopping Conspiracy', '2024', '84', '8.0', '2026-02-05', 'poster-1788974001-ea770d.png', '', '0', '2026-09-09 17:13:21'),
('498', 'Cortometraje', '0', 'Contra la Pared', '2023', '4', '7.0', '2026-02-07', 'poster-1788974032-be4412.png', '', '0', '2026-09-09 17:13:52'),
('499', 'Película', '0', 'Crime 101', '2026', '140', '6.0', '2026-02-13', 'poster-1788974055-b7e0ec.png', '', '0', '2026-09-09 17:14:15'),
('500', 'Película', '0', 'Alien: Covenant', '2017', '122', '6.0', '2026-02-14', 'poster-1788974077-ee6834.png', '', '0', '2026-09-09 17:14:37');
INSERT INTO `peliculas_series` (`id`, `categoria`, `es_serie`, `titulo`, `anio`, `duracion`, `nota`, `fecha_vista`, `poster`, `comentario`, `seleccion`, `creado`) VALUES
('501', 'Película', '0', 'City Lights', '1931', '87', '8.0', '2026-02-25', 'poster-1788975486-e99f39.png', '', '0', '2026-09-09 17:38:06'),
('502', 'Documental', '0', 'Manhunt: The Inside Story of the Hunt for Bin Laden', '2013', '102', '7.0', '2026-03-01', 'poster-1788975531-40f4ed.png', '', '0', '2026-09-09 17:38:51'),
('503', 'Película', '0', 'Modern Times', '1936', '87', '9.0', '2026-03-07', 'poster-1788975577-071b0f.png', '', '0', '2026-09-09 17:39:37'),
('504', 'Película', '0', 'The Great Dictator', '1940', '125', '7.0', '2026-03-08', 'poster-1788975600-404ae0.png', '', '0', '2026-09-09 17:40:00'),
('505', 'Película', '0', 'Hoppers', '2026', '104', '8.0', '2026-03-14', 'poster-1788975710-003188.png', '', '0', '2026-09-09 17:41:50'),
('506', 'Película', '0', 'Casablanca', '1942', '102', '9.0', '2026-03-16', 'poster-1788975739-d38d54.png', '', '0', '2026-09-09 17:42:19'),
('507', 'Película', '0', 'Project Hail Mary', '2026', '156', '8.0', '2026-03-20', 'poster-1788975788-3a42a3.png', '', '0', '2026-09-09 17:43:08'),
('508', 'Serie', '1', 'La Oficina', '2018', NULL, '8.0', '2026-03-22', 'poster-1788975832-e8f543.png', '', '0', '2026-09-09 17:43:52'),
('509', 'Película', '0', 'The Super Mario Galaxy Movie', '2026', '98', '7.0', '2026-04-19', 'poster-1788975863-2f0ba7.png', '', '0', '2026-09-09 17:44:23'),
('510', 'Película', '0', 'The Drama', '2026', '105', '7.0', '2026-04-19', 'poster-1788975896-a64b5b.png', '', '0', '2026-09-09 17:44:56'),
('511', 'Cortometraje', '0', 'The Punisher: One Last Kill', '2026', '48', '3.0', '2026-05-14', 'poster-1788977674-91f626.png', '', '0', '2026-09-09 18:14:34'),
('512', 'Película', '0', 'Sujo', '2024', '126', '7.0', '2026-05-16', 'poster-1788977713-3dd576.png', '', '0', '2026-09-09 18:15:13'),
('513', 'Película', '0', 'Michael', '2026', '127', '6.0', '2026-05-18', 'poster-1788977748-518789.png', '', '0', '2026-09-09 18:15:48'),
('514', 'Película', '0', 'Queer', '2024', '137', '8.0', '2026-05-26', 'poster-1788977770-a269da.png', '', '0', '2026-09-09 18:16:10'),
('515', 'Película', '0', 'Backrooms', '2026', '126', '6.0', '2026-05-30', 'poster-1788977795-c0e387.png', '', '0', '2026-09-09 18:16:35'),
('516', 'Película', '0', 'Amarga Navidad', '2026', '111', '7.0', '2026-06-06', 'poster-1788977831-8c9b0d.png', '', '0', '2026-09-09 18:17:11'),
('517', 'Película', '0', 'Familia a la Deriva', '2026', '98', '4.0', '2026-06-07', 'poster-1788977861-a41fb5.png', '', '0', '2026-09-09 18:17:41'),
('518', 'Película', '0', 'It\'s a Wonderful Life', '2026', '130', '9.0', '2026-06-12', 'poster-1788977944-0a89cd.png', '', '0', '2026-09-09 18:19:04'),
('519', 'Película', '0', 'México 86', '2026', '95', '8.0', '2026-06-14', 'poster-1788977968-b5fe69.png', '', '0', '2026-09-09 18:19:28'),
('520', 'Película', '0', 'Disclosure Day', '2026', '145', '8.0', '2026-06-17', 'poster-1788978005-7582f8.png', '', '0', '2026-09-09 18:20:05'),
('521', 'Película', '0', 'Toy Story 5', '2026', '102', '10.0', '2026-06-17', 'poster-1788978042-7d4956.png', '', '1', '2026-09-09 18:20:42'),
('522', 'Película', '0', 'Sunset Boulevard', '1950', '110', '7.0', '2026-06-19', 'poster-1788978093-61e174.png', '', '0', '2026-09-09 18:21:33'),
('523', 'Película', '0', 'Obsession', '2025', '109', '6.0', '2026-06-21', 'poster-1788978117-59330c.png', '', '0', '2026-09-09 18:21:57'),
('524', 'Película', '0', 'Singin\' in the Rain', '1952', '103', '6.0', '2026-06-21', 'poster-1788978144-ef2013.png', '', '0', '2026-09-09 18:22:24'),
('525', 'Película', '0', 'Ikiru', '1952', '143', '7.0', '2026-06-28', 'poster-1788978172-fbdf39.png', '', '0', '2026-09-09 18:22:52'),
('526', 'Película', '0', 'Seven Samurai', '1954', '207', '7.0', '2026-07-01', 'poster-1788978199-ad2ce2.png', '', '0', '2026-09-09 18:23:19'),
('527', 'Serie', '1', 'The Bear', '2022', NULL, '8.0', '2026-07-02', 'poster-1788978276-df147a.png', '', '0', '2026-09-09 18:24:36'),
('528', 'Película', '0', 'Rear Window', '1954', '112', '9.0', '2026-07-02', 'poster-1788978368-4a3736.png', '', '0', '2026-09-09 18:26:08'),
('529', 'Película', '0', '12 Angry Men', '1957', '96', '7.0', '2026-07-03', 'poster-1788978447-a0e87b.png', '', '0', '2026-09-09 18:27:27'),
('530', 'Película', '0', 'Minions & Monsters', '2026', '90', '7.0', '2026-07-04', 'poster-1788978472-4c77e0.png', '', '0', '2026-09-09 18:27:52'),
('531', 'Película', '0', 'Witness for the Prosecution', '1957', '116', '8.0', '2026-07-06', 'poster-1788978524-86ca7e.png', '', '0', '2026-09-09 18:28:44'),
('532', 'Película', '0', 'Paths of Glory', '1957', '88', '6.0', '2026-07-07', 'poster-1788978678-b9fc4a.png', '', '0', '2026-09-09 18:31:18'),
('533', 'Película', '0', 'Psycho', '1960', '109', '9.0', '2026-07-10', 'poster-1788978708-e6e0b2.png', '', '0', '2026-09-09 18:31:48'),
('534', 'Película', '0', 'The Invite', '2026', '107', '8.0', '2026-07-10', 'poster-1788978765-03b084.png', '', '0', '2026-09-09 18:32:45'),
('535', 'Película', '0', 'The Apartment', '1960', '125', '6.0', '2026-07-11', 'poster-1788978791-034d4c.png', '', '0', '2026-09-09 18:33:11'),
('536', 'Película', '0', 'Heartstopper Forever', '2026', '114', '8.0', '2026-07-18', 'poster-1788978851-dbf2f6.png', '', '0', '2026-09-09 18:34:11'),
('537', 'Película', '0', 'Harakiri', '1962', '133', '8.0', '2026-07-23', 'poster-1788978883-64f71f.png', '', '0', '2026-09-09 18:34:43'),
('538', 'Película', '0', 'The Odyssey', '2026', '172', '10.0', '2026-07-26', 'poster-1788978914-00c78f.png', '', '1', '2026-09-09 18:35:14'),
('539', 'Película', '0', 'High and Low', '1963', '143', '7.0', '2026-07-27', 'poster-1788978982-c7cb2e.png', '', '0', '2026-09-09 18:36:22'),
('540', 'Película', '0', 'Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb', '1964', '95', '6.0', '2026-07-29', 'poster-1788979025-81d230.png', '', '0', '2026-09-09 18:37:05'),
('541', 'Serie', '1', 'The Pitt', '2025', NULL, '10.0', '2026-07-31', 'poster-1788979054-1e1a24.png', '', '1', '2026-09-09 18:37:34'),
('542', 'Película', '0', 'Spider-Man: Brand New Day', '2026', '145', '9.0', '2026-08-03', 'poster-1788979085-f4c239.png', '', '0', '2026-09-09 18:38:05'),
('543', 'Película', '0', 'Magallanes', '2025', '160', '3.0', '2026-08-06', 'poster-1788979110-ce7f97.png', '', '0', '2026-09-09 18:38:30'),
('544', 'Película', '0', 'The Good, the Bad and the Ugly', '1966', '178', '10.0', '2026-08-09', 'poster-1788979144-5f11ce.png', '', '1', '2026-09-09 18:39:04'),
('545', 'Película', '0', 'Once Upon a Time in the West', '1968', '145', '7.0', '2026-08-10', 'poster-1788979172-8e912a.png', '', '0', '2026-09-09 18:39:32'),
('546', 'Película', '0', 'La Captura', '2026', '91', '7.0', '2026-09-04', 'poster-1788979193-08627b.png', '', '0', '2026-09-09 18:39:53'),
('547', 'Película', '0', 'Quinze Días', '2026', '100', '2.0', '2026-09-05', 'poster-1788979248-d76671.png', '', '0', '2026-09-09 18:40:48');

INSERT INTO `pelicula_personas` (`id`, `pelicula_id`, `nombre`, `orden`) VALUES
('1', '1', 'Oriol Paulo', '1'),
('2', '2', 'Steven Spielberg', '1'),
('3', '3', 'Bradley Cooper', '1'),
('4', '4', 'Anthony Salter', '1'),
('5', '5', 'David Benioff', '1'),
('6', '6', 'Patty Jenkins', '1'),
('7', '7', 'Malcolm D. Lee', '1'),
('8', '8', 'Jon Watts', '1'),
('9', '9', 'Francis Lawrence', '1'),
('10', '10', 'Destin Daniel Cretton', '1'),
('11', '11', 'Jon Turteltaub', '1'),
('12', '12', 'Jorge Ramírez Suárez', '1'),
('13', '13', 'Adam Wingard', '1'),
('14', '14', 'Cate Shortland', '1'),
('15', '15', 'Craig Mazin', '1'),
('16', '16', 'Laurie Nunn', '1'),
('17', '17', 'Noah Baumbach', '1'),
('18', '18', 'Brad Peyton', '1'),
('19', '19', 'Pete Docter', '1'),
('20', '20', 'Tim Burton', '1'),
('21', '21', 'Lana Wachowski', '1'),
('22', '21', 'Lilly Wachowski', '2'),
('23', '22', 'Andy Muschietti', '1'),
('24', '23', 'Louis Leterrier', '1'),
('25', '24', 'Jeffrey Wright', '1'),
('26', '25', 'Lana Wachowski', '1'),
('27', '25', 'Lilly Wachowski', '2'),
('28', '26', 'Martin Scorsese', '1'),
('29', '27', 'Phil Lord', '1'),
('30', '28', 'Gareth Edwards', '1'),
('31', '29', 'David Yates', '1'),
('32', '30', 'Hernán Guerschuny', '1'),
('33', '31', 'Raja Gosnell', '1'),
('34', '32', 'Joseph Kosinski', '1'),
('35', '33', 'Greg Berlanti', '1'),
('36', '34', 'Raúl Martínez', '1'),
('37', '35', 'David Fincher', '1'),
('38', '36', 'Quentin Tarantino', '1'),
('39', '37', 'Jac Schaeffer', '1'),
('40', '38', 'Jon Favreau', '1'),
('41', '39', 'Denis Villeneuve', '1'),
('42', '40', 'Jon Watts', '1'),
('43', '41', 'Eugenio Derbez', '1'),
('44', '42', 'Dennis Dugan', '1'),
('45', '43', 'Robert Zemeckis', '1'),
('46', '44', 'Chloé Zhao', '1'),
('47', '45', 'Todd Phillips', '1'),
('48', '46', 'Yeon Sang-ho', '1'),
('49', '47', 'John Krasinski', '1'),
('50', '48', 'Galder Gaztelu-Urrutia', '1'),
('51', '49', 'J. G. Quintel', '1'),
('52', '50', 'Steve Carr', '1'),
('53', '51', 'Felix van Groeningen', '1'),
('54', '52', 'Chris Columbus', '1'),
('55', '53', 'Lasse Hallstrom', '1'),
('56', '54', 'Sam Raimi', '1'),
('57', '55', 'Chava Cartas', '1'),
('58', '56', 'Sam Raimi', '1'),
('59', '57', 'Chris Columbus', '1'),
('60', '58', 'Lenny Abrahamson', '1'),
('61', '59', 'Dennis Dugan', '1'),
('62', '60', 'Paul Greengrass', '1'),
('63', '61', 'John Lee Hancock', '1'),
('64', '62', 'Todd Phillips', '1'),
('65', '63', 'Alfonso Cuarón', '1'),
('66', '64', 'Colin Trevorrow', '1'),
('67', '65', 'Sam Raimi', '1'),
('68', '66', 'Jordan Vogt-Roberts', '1'),
('69', '67', 'David Fincher', '1'),
('70', '68', 'Mark Osborne', '1'),
('71', '69', 'M. Night Shyamalan', '1'),
('72', '70', 'Jennifer Yuh Nelson', '1'),
('73', '71', 'Joe Johnston', '1'),
('74', '72', 'Christopher Nolan', '1'),
('75', '73', 'Alberto Rodríguez', '1'),
('76', '74', 'Tim Hill', '1'),
('77', '75', 'Betty Thomas', '1'),
('78', '76', 'Mike Mitchell', '1'),
('79', '77', 'David Fincher', '1'),
('80', '78', 'Todd Phillips', '1'),
('81', '79', 'Lin-Manuel Miranda', '1'),
('82', '80', 'Alfonso Cuarón', '1'),
('83', '81', 'Lana Wachowski', '1'),
('84', '81', 'Lilly Wachowski', '2'),
('85', '82', 'Todd Phillips', '1'),
('86', '83', 'Mike Newell', '1'),
('87', '84', 'Ben Bocquelet', '1'),
('88', '85', 'Jennifer Yuh Nelson', '1'),
('89', '86', 'Gino Nichele', '1'),
('90', '87', 'Jon Watts', '1'),
('91', '88', 'David Yates', '1'),
('92', '89', 'Jonathan Igla', '1'),
('93', '90', 'Steven Spielberg', '1'),
('94', '91', 'Lana Wachowski', '1'),
('95', '92', 'David Yates', '1'),
('96', '93', 'Aaron Hann', '1'),
('97', '94', 'Federico D\'Alessandro', '1'),
('98', '95', 'Adam McKay', '1'),
('99', '96', 'Humberto Hinojosa Ozcariz', '1'),
('100', '97', 'David Yates', '1');
INSERT INTO `pelicula_personas` (`id`, `pelicula_id`, `nombre`, `orden`) VALUES
('101', '98', 'David Yates', '1'),
('102', '99', 'Pierre Coffin', '1'),
('103', '100', 'Pierre Coffin', '1'),
('104', '101', 'J. A. Bayona', '1'),
('105', '102', 'Julian Schabel', '1'),
('106', '103', 'Walt Becker', '1'),
('107', '104', 'Henry Joost', '1'),
('108', '105', 'Nacho G. Velilla', '1'),
('109', '106', 'Ricardo Arnaiz', '1'),
('110', '107', 'Seth MacFarlane', '1'),
('111', '108', 'John Chu', '1'),
('112', '109', 'Wes Ball', '1'),
('113', '110', 'Wes Ball', '1'),
('114', '111', 'Seth MacFarlane', '1'),
('115', '112', 'Alex Hirsch', '1'),
('116', '113', 'Andy Muschietti', '1'),
('117', '114', 'Steven Spielberg', '1'),
('118', '115', 'Sebastián Hoffman', '1'),
('119', '116', 'Carlos Saldanha', '1'),
('120', '117', 'Rob Savage', '1'),
('121', '118', 'Ruben Fleischer', '1'),
('122', '119', 'Dennis Dugan', '1'),
('123', '120', 'David Fincher', '1'),
('124', '121', 'Gary Alazraki', '1'),
('125', '122', 'Sam Levinson', '1'),
('126', '123', 'James Gunn', '1'),
('127', '124', 'Quentin Tarantino', '1'),
('128', '125', 'Stanley Kubrick', '1'),
('129', '126', 'Tim Hill', '1'),
('130', '127', 'Desconocido', '1'),
('131', '128', 'Samuel Armstrong', '1'),
('132', '129', 'Carlos Carrera', '1'),
('133', '130', 'Justin Baldoni', '1'),
('134', '131', 'Robert Eggers', '1'),
('135', '132', 'Patricia Riggen', '1'),
('136', '133', 'David Fincher', '1'),
('137', '134', 'Clay Kaytis', '1'),
('138', '135', 'Gael García Bernal', '1'),
('139', '136', 'Harald Zwart', '1'),
('140', '137', 'Andy Fickman', '1'),
('141', '138', 'Sara Colangelo', '1'),
('142', '139', 'Gary Ross', '1'),
('143', '140', 'David Fincher', '1'),
('144', '141', 'Jane Campion', '1'),
('145', '142', 'Tony Ayres', '1'),
('146', '143', 'Greta Gerwig', '1'),
('147', '144', 'Jenny Gage', '1'),
('148', '145', 'Luca Guadagnino', '1'),
('149', '146', 'Guillermo del Toro', '1'),
('150', '147', 'Daniel Chong', '1'),
('151', '148', 'Manolo Caro', '1'),
('152', '149', 'Jonathan Liebesman', '1'),
('153', '150', 'Dave Wasson', '1'),
('154', '151', 'Andrew Stanton', '1'),
('155', '152', 'Roger Kumble', '1'),
('156', '153', 'Steven Spielberg', '1'),
('157', '154', 'James McTelgue', '1'),
('158', '155', 'Matt Reeves', '1'),
('159', '156', 'Alex Garland', '1'),
('160', '157', 'Anna Boden', '1'),
('161', '158', 'Ernesto Contreras', '1'),
('162', '159', 'Domee Shi', '1'),
('163', '160', 'M. Night Shyamalan', '1'),
('164', '161', 'Héctor Lozano', '1'),
('165', '162', 'Castille Landon', '1'),
('166', '163', 'Alejandro Springall', '1'),
('167', '164', 'Clint Eastwood', '1'),
('168', '165', 'Haigai Levi', '1'),
('169', '166', 'Sunghoo Park', '1'),
('170', '167', 'Peter Chelsom', '1'),
('171', '168', 'Peter Weir', '1'),
('172', '169', 'Oriol Paulo', '1'),
('173', '170', 'Rob Letterman', '1'),
('174', '171', 'Ari Aster', '1'),
('175', '172', 'Stanley Kubrick', '1'),
('176', '173', 'Ernesto Contreras', '1'),
('177', '174', 'Christopher Nolan', '1'),
('178', '175', 'Gustavo Loza', '1'),
('179', '176', 'David Twohy', '1'),
('180', '177', 'Toby Meakins', '1'),
('181', '178', 'Pedro Almodóvar', '1'),
('182', '179', 'David Lowery', '1'),
('183', '180', 'Adam Wingard', '1'),
('184', '181', 'Desconocido', '1'),
('185', '182', 'Carlos Santos', '1'),
('186', '183', 'Desconocido', '1'),
('187', '184', 'Desconocido', '1'),
('188', '185', 'Fernando Meirelles', '1'),
('189', '186', 'Tom McGrath', '1'),
('190', '187', 'Desconocido', '1'),
('191', '188', 'Yorgos Lanthimos', '1'),
('192', '189', 'Tom Ford', '1'),
('193', '190', 'Jeff Fowler', '1'),
('194', '191', 'Jeff Fowler', '1'),
('195', '192', 'Sam Raimi', '1'),
('196', '193', 'Jeremy Slater', '1'),
('197', '194', 'Steven Brill', '1'),
('198', '195', 'Cary Joji Fukunaga', '1'),
('199', '196', 'Stanley Kubrick', '1'),
('200', '197', 'Desconocido', '1');
INSERT INTO `pelicula_personas` (`id`, `pelicula_id`, `nombre`, `orden`) VALUES
('201', '198', 'Stephen Chbosky', '1'),
('202', '199', 'Pietro Loprieno', '1'),
('203', '200', 'Bong Joon-Ho', '1'),
('204', '201', 'Michael Dougherty', '1'),
('205', '202', 'Jon Favreau', '1'),
('206', '203', 'Pete Docter', '1'),
('207', '204', 'Erika J. Wood', '1'),
('208', '205', 'Ang Lee', '1'),
('209', '206', 'Raúl Campos', '1'),
('210', '207', 'Peter Sohn', '1'),
('211', '208', 'W. Kamau Bell', '1'),
('212', '209', 'Ulises Valencia', '1'),
('213', '210', 'Wes Anderson', '1'),
('214', '211', 'Raúl Campos', '1'),
('215', '212', 'Rich Moore', '1'),
('216', '213', 'Jeff Orlowski-Yang', '1'),
('217', '214', 'Ulises Valencia', '1'),
('218', '215', 'Colin Trevorrow', '1'),
('219', '216', 'Raúl Campos', '1'),
('220', '217', 'James Ford Murphy', '1'),
('221', '218', 'Carlos Saldanha', '1'),
('222', '219', 'Taika Waititi', '1'),
('223', '220', 'Denis Villeneuve', '1'),
('224', '221', 'Marcos Bucay', '1'),
('225', '222', 'James Gunn', '1'),
('226', '223', 'Robert Eggers', '1'),
('227', '224', 'Daniel Kwan', '1'),
('228', '225', 'Raúl Campos', '1'),
('229', '226', 'Robert Zemeckis', '1'),
('230', '227', 'Robert Eggers', '1'),
('231', '228', 'Phil Lord', '1'),
('232', '229', 'Julian Gaviria', '1'),
('233', '230', 'Stan Lathan', '1'),
('234', '231', 'Kevin Hench', '1'),
('235', '232', 'Bob Persichetti', '1'),
('236', '233', 'Joseph Kosinski', '1'),
('237', '234', 'Alex Díaz', '1'),
('238', '235', 'Akiva Schaffer', '1'),
('239', '236', 'Pierre Coffin', '1'),
('240', '237', 'Brad Ableson', '1'),
('241', '238', 'Adriano Zumbo', '1'),
('242', '239', 'Raúl Campos', '1'),
('243', '240', 'Drew Goddard', '1'),
('244', '241', 'Todd Phillips', '1'),
('245', '242', 'Taika Waititi', '1'),
('246', '243', 'Miguel García Moreno', '1'),
('247', '244', 'Desconocido', '1'),
('248', '245', 'Dean DeBlois', '1'),
('249', '246', 'Brad Bird', '1'),
('250', '247', 'Brad Bird', '1'),
('251', '248', 'Lorcan Finnegan', '1'),
('252', '249', 'Baz Luhrmann', '1'),
('253', '250', 'Francis Ford Coppola', '1'),
('254', '251', 'Adam Wingard', '1'),
('255', '252', 'Barry Jenkins', '1'),
('256', '253', 'Chloé Zhao', '1'),
('257', '254', 'Vince Gilligan', '1'),
('258', '255', 'Vince Gilligan', '1'),
('259', '256', 'James Wan', '1'),
('260', '257', 'Jordan Peele', '1'),
('261', '258', 'Andy Serkis', '1'),
('262', '259', 'M. Night Shyamalan', '1'),
('263', '260', 'Gary Alazraki', '1'),
('264', '261', 'Tetsuro Kodama', '1'),
('265', '262', 'Jordan Peele', '1'),
('266', '263', 'Tom Hooper', '1'),
('267', '264', 'Francisco Schultz', '1'),
('268', '265', 'Olivia Wilde', '1'),
('269', '266', 'Ryan J. Condal', '1'),
('270', '267', 'Claude Shires', '1'),
('271', '268', 'Rodrigo Santos', '1'),
('272', '269', 'Manny Rodríguez', '1'),
('273', '270', 'Desconocido', '1'),
('274', '271', 'Ryan Coogler', '1'),
('275', '272', 'Desconocido', '1'),
('276', '273', 'James Gunn', '1'),
('277', '274', 'James Cameron', '1'),
('278', '275', 'Luca Guadagnino', '1'),
('279', '276', 'Norman Fergusson', '1'),
('280', '277', 'Raúl Campos', '1'),
('281', '278', 'Raúl Campos', '1'),
('282', '279', 'James Cameron', '1'),
('283', '280', 'Raúl Campos', '1'),
('284', '281', 'Guillermo del Toro', '1'),
('285', '282', 'Lucas Santa Ana', '1'),
('286', '283', 'Desconocido', '1'),
('287', '284', 'Jeff Gipson', '1'),
('288', '285', 'Domee Shi', '1'),
('289', '286', 'Alan Barillaro', '1'),
('290', '287', 'Kristen Lester', '1'),
('291', '288', 'Bobby Rubio', '1'),
('292', '289', 'Doug Sweetland', '1'),
('293', '290', 'Ken Marino', '1'),
('294', '291', 'Desconocido', '1'),
('295', '292', 'Desconocido', '1'),
('296', '293', 'Raúl Campos', '1'),
('297', '294', 'Desconocido', '1'),
('298', '295', 'Ruben Ostlund', '1'),
('299', '296', 'Angus McLane', '1'),
('300', '297', 'Desconocido', '1');
INSERT INTO `pelicula_personas` (`id`, `pelicula_id`, `nombre`, `orden`) VALUES
('301', '298', 'Carlos Montero', '1'),
('302', '299', 'Damien Chazelle', '1'),
('303', '300', 'Raúl Campos', '1'),
('304', '301', 'Park Chan-wook', '1'),
('305', '302', 'Steven Spielberg', '1'),
('306', '303', 'Guillem Clua', '1'),
('307', '304', 'Peyton Reed', '1'),
('308', '305', 'Pedro Solís García', '1'),
('309', '306', 'Todd Field', '1'),
('310', '307', 'Darren Aronofsky', '1'),
('311', '308', 'Neil Druckmann', '1'),
('312', '309', 'Michael B. Jordan', '1'),
('313', '310', 'Joel Gallen', '1'),
('314', '311', 'Vasilis Katsoupis', '1'),
('315', '312', 'James Gunn', '1'),
('316', '313', 'Desconocido', '1'),
('317', '314', 'Jonah Hill', '1'),
('318', '315', 'Duncan Bridgeman', '1'),
('319', '316', 'Joshua Rofé', '1'),
('320', '317', 'Trisha Ziff', '1'),
('321', '318', 'Aaron Horvath', '1'),
('322', '319', 'Enrique García Meza', '1'),
('323', '320', 'Felicity Morris', '1'),
('324', '321', 'Noah Baumbach', '1'),
('325', '322', 'Maiken Baird', '1'),
('326', '323', 'Desconocido', '1'),
('327', '324', 'Dan Schneider', '1'),
('328', '325', 'Jenny Popplewell', '1'),
('329', '326', 'Justin K. Thompson', '1'),
('330', '327', 'David Broome', '1'),
('331', '328', 'Tom Six', '1'),
('332', '329', 'Mike Binder', '1'),
('333', '330', 'Louis Leterrier', '1'),
('334', '331', 'Andy Muschietti', '1'),
('335', '332', 'Alex Díaz', '1'),
('336', '333', 'Raúl Campos', '1'),
('337', '334', 'Michael Govier', '1'),
('338', '335', 'Frank E. Abney III', '1'),
('339', '336', 'Pedro Ample', '1'),
('340', '337', 'Jay Oliva', '1'),
('341', '338', 'Jay Oliva', '1'),
('342', '339', 'Ashley Eakin', '1'),
('343', '340', 'Ari Aster', '1'),
('344', '341', 'Denis Villeneuve', '1'),
('345', '342', 'Julio César Gaviria', '1'),
('346', '343', 'Alejandro G. Iñárritu', '1'),
('347', '344', 'Craig Johnson', '1'),
('348', '345', 'Yorgos Lanthimos', '1'),
('349', '346', 'Cristopher McQuarrie', '1'),
('350', '347', 'Christopher Nolan', '1'),
('351', '348', 'John Krasinski', '1'),
('352', '349', 'María José Cuevas', '1'),
('353', '350', 'Greta Gerwig', '1'),
('354', '351', 'Fernando Frias', '1'),
('355', '352', 'Danny Philippou', '1'),
('356', '352', 'Michael Philippou', '2'),
('357', '353', 'Desconocido', '1'),
('358', '354', 'Joe Wright', '1'),
('359', '355', 'Gavin Hood', '1'),
('360', '356', 'Josh Greenbaum', '1'),
('361', '357', 'Kevin Greutert', '1'),
('362', '358', 'Emma Tammi', '1'),
('363', '359', 'Sarah Polley', '1'),
('364', '360', 'M. Night Shyamalan', '1'),
('365', '361', 'Santiago Limón', '1'),
('366', '362', 'David Fincher', '1'),
('367', '363', 'Francis Lawrence', '1'),
('368', '364', 'Paul King', '1'),
('369', '365', 'Wes Anderson', '1'),
('370', '366', 'Ridley Scott', '1'),
('371', '367', 'Lila Avilés', '1'),
('372', '368', 'Héctor Lozano', '1'),
('373', '369', 'James Wan', '1'),
('374', '370', 'J. A. Bayona', '1'),
('375', '371', 'John L. Spencer', '1'),
('376', '372', 'Everardo González', '1'),
('377', '373', 'Emerald Fennell', '1'),
('378', '374', 'Demián Rugna', '1'),
('379', '375', 'Kristóf Deák', '1'),
('380', '376', 'Patricia Font', '1'),
('381', '377', 'David Maddox', '1'),
('382', '378', 'Cristopher Zalla', '1'),
('383', '379', 'Raúl Campos', '1'),
('384', '380', 'Denis Villeneuve', '1'),
('385', '381', 'Lars Beckung', '1'),
('386', '382', 'Misan Harriman', '1'),
('387', '383', 'Greg Berlanti', '1'),
('388', '384', 'Matthew A. Cherry', '1'),
('389', '385', 'Desconocido', '1'),
('390', '386', 'Wes Ball', '1'),
('391', '387', 'Lukas Dhont', '1'),
('392', '388', 'Dev Patel', '1'),
('393', '389', 'Franco Escamilla', '1'),
('394', '390', 'Wim Wenders', '1'),
('395', '391', 'Louis Leterrier', '1'),
('396', '392', 'Jon M. Chu', '1'),
('397', '393', 'Pedro Almodóvar', '1'),
('398', '394', 'Kelsey Mann', '1'),
('399', '395', 'Bae Jin Soo', '1'),
('400', '396', 'Hirokazu Koreeda', '1');
INSERT INTO `pelicula_personas` (`id`, `pelicula_id`, `nombre`, `orden`) VALUES
('401', '397', 'Patrick Delage', '1'),
('402', '398', 'Michael Sarnoski', '1'),
('403', '399', 'Charlotte Wells', '1'),
('404', '400', 'Shawn Levy', '1'),
('405', '401', 'Jerzy Skolimowski', '1'),
('406', '402', 'Georges Méliès', '1'),
('407', '403', 'M. Night Shyamalan', '1'),
('408', '404', 'Fede Alvarez', '1'),
('409', '405', 'Chris Sanders', '1'),
('410', '406', 'Desconocido', '1'),
('411', '407', 'Galder Gaztelu-Urrutia', '1'),
('412', '408', 'Alice Oseman', '1'),
('413', '409', 'Ida Cuéllar', '1'),
('414', '410', 'John Crowley', '1'),
('415', '411', 'Jeff Fowler', '1'),
('416', '412', 'Gints Zilbalodis', '1'),
('417', '413', 'Yarrow Cheney', '1'),
('418', '414', 'Alex Garland', '1'),
('419', '415', 'Desconocido', '1'),
('420', '416', 'Cameron Caimes', '1'),
('421', '417', 'Manny Rodríguez', '1'),
('422', '418', 'Adam Wingard', '1'),
('423', '419', 'Mariana Cenillo', '1'),
('424', '420', 'Carlos Pérez Osorio', '1'),
('425', '421', 'Desconocido', '1'),
('426', '422', 'Desconocido', '1'),
('427', '423', 'Bong Joon-Ho', '1'),
('428', '424', 'Desconocido', '1'),
('429', '425', 'Coralie Fargeat', '1'),
('430', '426', 'Desconocido', '1'),
('431', '427', 'Carlos Santos', '1'),
('432', '428', 'Edward Berger', '1'),
('433', '429', 'Simón Hernández', '1'),
('434', '430', 'Stephen Graham', '1'),
('435', '431', 'Desconocido', '1'),
('436', '432', 'Desconocido', '1'),
('437', '433', 'Peter Browngardt', '1'),
('438', '434', 'Wim Wenders', '1'),
('439', '435', 'Jake Schreier', '1'),
('440', '436', 'Ernesto Pagano', '1'),
('441', '437', 'Juliana Fanjul', '1'),
('442', '438', 'Charlie Brooker', '1'),
('443', '439', 'Jonathan Entwistle', '1'),
('444', '440', 'Cristopher McQuarrie', '1'),
('445', '441', 'Desconocido', '1'),
('446', '442', 'Desconocido', '1'),
('447', '443', 'Alejandro G. Iñárritu', '1'),
('448', '444', 'Anthony Russo', '1'),
('449', '444', 'Joe Russo', '2'),
('450', '445', 'Samuel Armstrong', '1'),
('451', '446', 'Kevin Nolting', '1'),
('452', '447', 'Brian de Palma', '1'),
('453', '448', 'John Woo', '1'),
('454', '449', 'Choi Jin-sung', '1'),
('455', '450', 'Dean DeBlois', '1'),
('456', '451', 'Dean DeBlois', '1'),
('457', '452', 'Hwang Dong-hyuk', '1'),
('458', '453', 'Joseph Kosinski', '1'),
('459', '454', 'James Gunn', '1'),
('460', '455', 'J. J. Abrams', '1'),
('461', '456', 'Francis Lawrence', '1'),
('462', '457', 'Gareth Edwards', '1'),
('463', '458', 'Joe Berlinger', '1'),
('464', '459', 'Francis Lawrence', '1'),
('465', '460', 'Matt Shakman', '1'),
('466', '461', 'Spike Jonze', '1'),
('467', '462', 'Alonso Ruizpalacios', '1'),
('468', '463', 'Pierre Perifel', '1'),
('469', '464', 'Celine Song', '1'),
('470', '465', 'Danny Philippou', '1'),
('471', '465', 'Michael Philippou', '2'),
('472', '466', 'Scott Derrickson', '1'),
('473', '467', 'Brad Bird', '1'),
('474', '468', 'Paul Thomas Anderson', '1'),
('475', '469', 'Álex Pina', '1'),
('476', '470', 'Joachim Ronning', '1'),
('477', '471', 'Luca Guadagnino', '1'),
('478', '472', 'Ruben Fleischer', '1'),
('479', '473', 'Joss Whedon', '1'),
('480', '474', 'Guillermo del Toro', '1'),
('481', '475', 'Zach Cregger', '1'),
('482', '476', 'Iain Morris', '1'),
('483', '477', 'Jafar Panahi', '1'),
('484', '478', 'Yorgos Lanthimos', '1'),
('485', '479', 'Emma Tammi', '1'),
('486', '480', 'Rich Moore', '1'),
('487', '481', 'Piero Messina', '1'),
('488', '482', 'James Cameron', '1'),
('489', '483', 'Fran Hevia', '1'),
('490', '484', 'Ariel Winograd', '1'),
('491', '485', 'Joachim Trier', '1'),
('492', '486', 'Iván Escobar', '1'),
('493', '487', 'Desconocido', '1'),
('494', '488', 'Mark Mylod', '1'),
('495', '489', 'Rikki Hughes', '1'),
('496', '490', 'Hikari', '1'),
('497', '491', 'Sean Baker', '1'),
('498', '492', 'Park Chan-wook', '1'),
('499', '493', 'Josh Safdie', '1'),
('500', '494', 'Raúl Campos', '1');
INSERT INTO `pelicula_personas` (`id`, `pelicula_id`, `nombre`, `orden`) VALUES
('501', '495', 'Desconocido', '1'),
('502', '496', 'Chloé Zhao', '1'),
('503', '497', 'Nic Stacey', '1'),
('504', '498', 'Fran Hevia', '1'),
('505', '499', 'Bart Layton', '1'),
('506', '500', 'Ridley Scott', '1'),
('507', '501', 'Charles Chaplin', '1'),
('508', '502', 'Greg Barker', '1'),
('509', '503', 'Charles Chaplin', '1'),
('510', '504', 'Charles Chaplin', '1'),
('511', '505', 'Daniel Chong', '1'),
('512', '506', 'Michael Curtiz', '1'),
('513', '507', 'Phil Lord', '1'),
('514', '508', 'Desconocido', '1'),
('515', '509', 'Aaron Horvath', '1'),
('516', '510', 'Kristoffer Borgli', '1'),
('517', '511', 'Reinaldo Marcus Green', '1'),
('518', '512', 'Astris Rondero', '1'),
('519', '513', 'Antoine Fuqua', '1'),
('520', '514', 'Luca Guadagnino', '1'),
('521', '515', 'Kane Parsons', '1'),
('522', '516', 'Pedro Almodóvar', '1'),
('523', '517', 'Alfonso Pineda Ulloa', '1'),
('524', '518', 'Frank Capra', '1'),
('525', '519', 'Gabriel Ripstein', '1'),
('526', '520', 'Steven Spielberg', '1'),
('527', '521', 'McKenna Harris', '1'),
('528', '522', 'Billy Wilder', '1'),
('529', '523', 'Curry Barker', '1'),
('530', '524', 'Gene Kelley', '1'),
('531', '525', 'Akira Kurosawa', '1'),
('532', '526', 'Akira Kurosawa', '1'),
('533', '527', 'Christopher Storer', '1'),
('534', '528', 'Alfred Hitchcock', '1'),
('535', '529', 'Sidney Lumet', '1'),
('536', '530', 'Pierre Coffin', '1'),
('537', '531', 'Billy Wilder', '1'),
('538', '532', 'Stanley Kubrick', '1'),
('539', '533', 'Alfred Hitchcock', '1'),
('540', '534', 'Olivia Wilde', '1'),
('541', '535', 'Billy Wilder', '1'),
('542', '536', 'Wash Westmoreland', '1'),
('543', '537', 'Masaki Kobayashi', '1'),
('544', '538', 'Christopher Nolan', '1'),
('545', '539', 'Akira Kurosawa', '1'),
('546', '540', 'Stanley Kubrick', '1'),
('547', '541', 'R. Scott Gemmill', '1'),
('548', '542', 'Destin Daniel Cretton', '1'),
('549', '543', 'Lav Díaz', '1'),
('550', '544', 'Sergio Leone', '1'),
('551', '545', 'Sergio Leone', '1'),
('552', '546', 'Chava Cartas', '1'),
('553', '547', 'Daniel Lieff', '1');
...
-- `visitas` cuenta exactamente las visitas a la portada, día por día: es el
-- histórico diario de `/`. Copiarlo hace que la tabla «Páginas más visitadas»
-- cuadre con la gráfica en cualquier periodo. IGNORE: los días que la tabla
-- ya registró por sí sola se conservan tal cual.
INSERT IGNORE INTO `visitas_pagina_dia` (`ruta`, `fecha`, `total`)
...
-- visitas_pagina_dia: Inicio se rellena arriba desde `visitas`; el resto de
-- rutas empieza el día del despliegue (antes solo hay total en visitas_pagina).
